...
function validateDataStructure(data, expectedKeys, source) {
  if (!data || typeof data !== "object") {
    throw new Error(
      `${source}: Invalid data structure - expected object, got ${typeof data}`,
    );
  }
  ...
  if (missingKeys.length > 0) {
    throw new Error(
      `${source}: Missing required keys: ${missingKeys.join(", ")}`,
    );
  }
...
const CURRENT_SEASON =
  Number(process.env.NFL_SEASON) || new Date().getUTCFullYear();
const VERBOSE_LOGS =
...
const FP_ECR_OUT_PATH = path.join(
  __dirname,
  "../public/fantasypros_ecr_cache.json",
);
...
const VALUE_OVERRIDES_PATH = path.join(
  __dirname,
  "../data/value-overrides.json",
);
const VALUE_CACHE_VERSION_PATH = path.join(
  __dirname,
  "../public/value-cache-version.json",
);
const PROJ_IN_PATH =
  process.env.PROJ_CSV ||
  path.join(__dirname, `../data/projections_${CURRENT_SEASON}.csv`);
const PROJ_OUT_PATH = path.join(
  __dirname,
  `../public/projections_${CURRENT_SEASON}.json`,
);
const ESPN_PROJ_OUT_PATH = path.join(
  __dirname,
  `../public/projections_espn_${CURRENT_SEASON}.json`,
);
const CBS_PROJ_OUT_PATH = path.join(
  __dirname,
...
const FANTASYSHARKS_PROJ_OUT_PATH = path.join(
  __dirname,
  `../public/projections_fantasysharks_${CURRENT_SEASON}.json`,
);
const DRAFTSHARKS_PROJ_OUT_PATH = path.join(
  __dirname,
  `../public/projections_draftsharks_${CURRENT_SEASON}.json`,
);
const FANTASYPROS_PROJ_OUT_PATH = path.join(
  __dirname,
  `../public/projections_fantasypros_${CURRENT_SEASON}.json`,
);
const ARSENAL_PROJ_OUT_PATH = path.join(
  __dirname,
...
function recordSourceFreshness(task, status = "success", error = "") {
  ...
  ledger.sources = {
    ...
    [task.key]: {
      ...
      last_attempt_at: now,
      last_success_at:
        status === "success" ? now : previous.last_success_at || null,
      last_error:
        status === "success"
          ? ""
          : String(error || "Update failed").slice(0, 300),
    },
  };
...
function archiveUpdatedValues(failures = []) {
  ...
  fs.writeFileSync(
    path.join(ARCHIVE_DIR, manifestFile),
  ...
  try {
    ...
    priorEntries = Array.isArray(priorIndex?.archives)
      ? priorIndex.archives
      : [];
  } catch {
  ...
  fs.writeFileSync(
    indexPath,
    JSON.stringify({ updated_at: createdAt, archives }, null, 2),
  );
  console.log(
    `📚 Archived ${archived.length} dated value/projection files for ${date}.`,
  );
}
...
function loadValueOverrides() {
  try {
    ...
    return {
      ...
      positionCorrections: Array.isArray(parsed?.positionCorrections)
        ? parsed.positionCorrections
        : [],
    };
  } catch (error) {
...
function applyValueOverridesToData(data, label = "value cache") {
  ...
  const visit = (node, inheritedName = "") => {
    ...
    const correction = overrides.positionCorrections.find(
      (row) => normalizedName(row.name) === nameKey,
    );
    if (correction && nameKey) {
      ...
      if (nextPosition && originalPosition !== nextPosition) {
        if ("position" in node || (!("pos" in node) && !("_position" in node)))
        ...
        if (node.player && typeof node.player === "object")
          node.player.position = nextPosition;
        corrected += 1;
      }
    }
    ...
    const effectivePosition = correction
      ...
      : originalPosition;
    const exclusion = overrides.zeroValues.find(
      (row) =>
  ...
  if (zeroed || corrected)
    console.log(
      `  Applied overrides to ${label}: ${corrected} position correction(s), ${zeroed} value field(s) zeroed.`,
    );
  return data;
}
...
function applyValueOverridesToAllCaches() {
  ...
  ].forEach(([filePath, label, compact]) =>
    applyValueOverridesToFile(filePath, label, compact),
  );
}

function writeValueCacheVersion() {
  fs.writeFileSync(
    VALUE_CACHE_VERSION_PATH,
    JSON.stringify(
      {
...
async function enableAdBlockLite(page, opts = {}) {
  ...
  page.on("request", (req) => {
    try {
      ...
      // Allow “always allow”
      if (ALWAYS_ALLOW_HOST_PARTS.some((p) => host.includes(p)))
        return req.continue();
...
function makePptrLaunchOpts() {
  ...
  const devtools =
    process.env.PPTR_DEVTOOLS === "1" || getArgFlag("--devtools");
...
const WANT_SHOTS = process.env.PPTR_SHOTS === "1" || getArgFlag("--shots");
const KEEP_OPEN = process.env.PPTR_KEEP === "1" || getArgFlag("--keep-open");
...
async function scrapeKTC(superflex = true) {
  ...
  await page.waitForSelector(".sf-toggle-wrapper.superflex .sf-toggle", {
    timeout: 50000,
  });
  await page.evaluate((isSF) => {
    const toggle = document.querySelector(
      ".sf-toggle-wrapper.superflex .sf-toggle",
    );
  ...
  const options = await page.$$eval(
    "#ranking-pagination-dropdown option",
    (opts) => opts.map((o) => ({ value: o.value, text: o.textContent.trim() })),
  );

  for (const opt of options) {
    await page.select("#ranking-pagination-dropdown", opt.value);
    await page.waitForSelector("#rankings-page-rankings .onePlayer", {
      timeout: 60000,
    });
    await sleep(800);
    ...
    const playersOnPage = await page.evaluate(() => {
      return Array.from(
        document.querySelectorAll("#rankings-page-rankings .onePlayer"),
      ).map((el) => ({
        rank: parseInt(
          el.querySelector(".rank-number p")?.innerText.trim() || "0",
        ),
        name: el.querySelector(".player-name a")?.innerText.trim() || "",
        team:
          el.querySelector(".player-name .player-team")?.innerText.trim() || "",
        position: (
          el.querySelector(".position-team .position")?.innerText || ""
        )
          .replace(/\d+$/, "")
          .trim(),
  ...
  console.log(
    `✅ Collected ${allPlayers.length} players (${superflex ? "SF" : "1QB"})`,
  );
  return allPlayers;
}
...
async function updateFantasyCalc() {
  ...
  const compact = Object.fromEntries(
    combinations.map(({ key }) => [key, new Map()]),
  );
  ...
  for (let i = 0; i < requests.length; i++) {
    ...
    logProgress(
      `  📡 Fetching FantasyCalc ${key} ${profile.key}`,
      i + 1,
      requests.length,
    );
    ...
    try {
      ...
      (Array.isArray(response.data) ? response.data : []).forEach((row) => {
        ...
        const identity = String(
          player?.sleeperId ||
            `${normName(player?.name || row?.name)}|${normalizePos(player?.position || row?.position)}`,
        );
        if (!identity) return;
  ...
  if (Object.keys(results).length === 0) {
    const total = Object.values(compact).reduce(
  ...
  console.log(
    `✅ fantasycalc_cache.json updated with ${requests.length} verified format/scoring boards.`,
  );
}
...
async function updateDynastyProcess() {
  ...
  parsed.forEach((row, index) => {
    try {
      ...
      console.warn(
        `  ⚠️ Error processing DynastyProcess row ${index + 1}:`,
        error.message,
      );
    }
  });
  ...
  console.log(
    `✅ dynastyprocess_cache.json updated (${processedCount} players processed).`,
  );
}
...
async function updateFantasyNavigator() {
  ...
  const latestByFormat = Object.fromEntries(
    Object.keys(results).map((key) => [key, new Map()]),
  );
  data.forEach((row) => {
  ...
  Object.keys(results).forEach((key) => {
    results[key] = [...latestByFormat[key].values()].sort(
      (a, b) => b.value - a.value,
    );
    const newest =
      results[key]
        ...
        .at(-1) || "unknown";
    console.log(
      `  ${key}: ${results[key].length} latest rows (newest source date ${newest})`,
    );
  });
...
async function updateFantasyPros() {
  const categoryUrl =
    "https://www.fantasypros.com/content/nfl/dynasty-nfl/nfl-trade-value-chart/";
  const category = await axios.get(categoryUrl, {
    timeout: CONFIG.REQUEST_TIMEOUT,
    headers: { "user-agent": "Mozilla/5.0", accept: "text/html" },
  });
  const articleMatches = [
    ...String(category.data || "").matchAll(
      /href=["']([^"']*fantasy-football-rankings-dynasty-trade-value-chart-[^"']*update\/?)["']/gi,
    ),
  ].map((match) => new URL(match[1], categoryUrl).href);
  const articleUrl = articleMatches[0];
  if (!articleUrl)
    throw new Error(
      "FantasyPros latest dynasty trade-value article was not found.",
    );
  ...
  const chartIds = [
    ...html.matchAll(/datawrapper\.dwcdn\.net\/([A-Za-z0-9]+)\/\d+/g),
  ].map((match) => match[1]);
  const uniqueIds = [...new Set(chartIds)].slice(0, 4);
  if (uniqueIds.length < 4)
    throw new Error(
      `FantasyPros article exposed ${uniqueIds.length}/4 expected value charts.`,
    );
  ...
  const sourceYear =
    Number(title.match(/\b(20\d{2})\b/)?.[1]) || CURRENT_SEASON;
  ...
  const output = {
    ...
    chart_ids: uniqueIds,
    scale_note:
      "FantasyPros published values multiplied by 100 for Arsenal tool-scale compatibility; source_value preserves the exact published number.",
  };
  ...
  for (let index = 0; index < uniqueIds.length; index += 1) {
    ...
    parsed.forEach((row) => {
      ...
      const tepRaw =
        positions[index] === "TE" ? Number(row["TEP Value"]) || raw : raw;
      const common = {
        name,
        ...
        tep_source_value: tepRaw,
        tep_value: Math.round(tepRaw * 100),
        source_date: sourceDate,
        chart_id: chartId,
      };
      output.Dynasty_1QB.push(common);
      const sfRaw =
        positions[index] === "QB" ? Number(row["SF Value"]) || raw : raw;
      output.Dynasty_SF.push({
        ...common,
        source_value: sfRaw,
        value: Math.round(sfRaw * 100),
        tep_source_value: positions[index] === "QB" ? sfRaw : tepRaw,
        tep_value: Math.round(
          (positions[index] === "QB" ? sfRaw : tepRaw) * 100,
        ),
      });
    });
  }
  ...
  console.log(
    `✅ fantasypros_cache.json updated (${output.Dynasty_SF.length} players, publisher date ${sourceDate}).`,
  );
}
...
async function updateFantasyProsECR() {
  ...
  if (!apiKey)
    throw new Error("FANTASYPROS_API_KEY is required for FantasyPros ECR.");
  ...
  const variants = formats.flatMap((format) =>
    (format.scoringSpecific
      ? scoringTypes
      : [{ key: "neutral", api: "PPR" }]
    ).map((scoring) => ({ ...format, scoring })),
  );
  const output = {
    updated: new Date().toISOString(),
    ...
    season: CURRENT_SEASON,
    methodology:
      "Actual FantasyPros ECR order converted to a 10,000-to-100 rank score for Arsenal display compatibility. rank_ecr preserves the published ordinal rank. Dynasty ECR is scoring-neutral; redraft has distinct STD, HALF, and PPR boards. This score is not a FantasyPros trade value and is not inherently additive.",
    formats: {},
    experts_by_format: {},
  ...
  for (const variant of variants) {
        ...
        if (
          ...
          sourceRows.length < Math.min(100, declaredCount)
        ) {
          throw new Error(
            `FantasyPros ECR ${variant.key} ${variant.scoring.api} coverage is incomplete (${sourceRows.length}/${declaredCount}).`,
          );
        }
        const listKey = variant.scoringSpecific
          ? `${variant.key}_${variant.scoring.api}`
          : variant.key;
        ...
        const rows = sourceRows
          .map((row, index) => {
            ...
            const value = Math.max(
              ...
              Math.round(
                10000 -
                  ((Math.max(1, rank) - 1) / Math.max(1, total - 1)) * 9900,
              ),
            );
            return {
              name: String(row?.player_name || "").trim(),
              position: String(
                row?.player_position_id || row?.player_positions || "",
              ).toUpperCase(),
        ...
        results.push({
          ...
          experts: {
            total:
              Number(payload?.total_experts) || Object.keys(expertNames).length,
            ...
            ranking_type:
              payload?.type || payload?.ranking_type_name || variant.type,
            position: variant.position,
  ...
  if (counts.length !== 8)
    throw new Error(
      `FantasyPros ECR produced ${counts.length}/8 expected unique format tables.`,
    );
  fs.writeFileSync(FP_ECR_OUT_PATH, JSON.stringify(output));
  console.log(
    `✅ fantasypros_ecr_cache.json updated (8 unique boards, ${counts.reduce((sum, count) => sum + count, 0)} ranked rows).`,
  );
}
...
function parseGvizResponseText(text) {
  ...
  const m =
    s.match(
      /google\.visualization\.Query\.setResponse\(\s*([\s\S]*?)\s*\)\s*;?\s*$/,
...
function parseIdpSheetToRows(gvizObj) {
  ...
  if (nameIdx < 0 || valIdx < 0) {
    const colList = cols
      ...
      .join(", ");
    throw new Error(
      `IDynastyP GViz missing required columns (need name + value). Found: ${colList}`,
    );
  }
  ...
  for (const r of rows) {
    ...
    const positionRaw =
      posIdx >= 0 ? String(cellValue(c[posIdx]) || "").trim() : "";
...
async function updateIDynastyP() {
  const url =
    "https://script.google.com/macros/s/AKfycbwn_NRprftHtibfVrnjIt-xzNV1uQdeJKccUklMCnJjv9mQgor5Ru3qGs1RKrfvpFmn/exec";
  console.log("Fetching IDynastyP first-party values feed...");
  const response = await axios.get(url, { timeout: 120000, family: 4 });
  const byName = new Map();
  coerceSheets(response.data)
  ...
  console.log(
    `✅ idynastyp_cache.json updated from GViz with ${normalized.length} entries.`,
  );
}
...
function coerceSheets(payload) {
  ...
  if (payload && typeof payload === "object") {
    const entries = Object.entries(payload).filter(
      ([k, v]) =>
        k !== "updated" && k !== "meta" && k !== "version" && Array.isArray(v),
    );
    if (entries.length) {
...
function normalizeRowToIdpShowRow(row) {
  ...
  const position = String(
    pickFirst(row, ["position", "pos", "Position", "POS"]),
  )
    .replace(/\d+$/, "")
    .trim();

  const team = String(
    pickFirst(row, ["team", "nfl_team", "Team", "TEAM", "club"]),
  )
    .trim()
    .toUpperCase();
  ...
  const v1qb = safeNum(
    pickFirst(row, ["value_1qb", "val_1qb", "1qb", "one_qb"]),
  );
  const vsf = safeNum(
    pickFirst(row, ["value_sf", "val_sf", "sf", "superflex"]),
  );
  const vtep = safeNum(pickFirst(row, ["value_tep", "val_tep", "tep"]));
  const vsftep = safeNum(
    pickFirst(row, ["value_sftep", "val_sftep", "sftep", "sf_tep"]),
  );
...
async function updateIDPShow() {
  ...
  if (typeof payload === "string") {
    try {
      ...
      throw new Error(
        "IDPShow: response was not valid JSON (see raw snippet above).",
      );
    }
  }
  ...
  if (payload && typeof payload === "object") {
    ...
    if (payload.sheets)
      console.log(
        "IDPShow sheets keys:",
        Object.keys(payload.sheets).slice(0, 30),
      );
  }
  ...
  if (!sheets.length) {
    throw new Error(
      "IDPShow: unexpected response shape (could not find any sheet arrays).",
    );
  }
  ...
  const isRedraftSheet = (sheetName) => {
    ...
    return (
      sn.includes("redraft") ||
      sn.includes("re-draft") ||
      sn.includes("seasonal")
    );
  };
  ...
  for (const { sheetName, rows } of sheets) {
    ...
    for (const r of rows) {
      try {
        ...
        console.warn(
          `    ⚠️ Error processing row in sheet ${sheetName}:`,
          error.message,
        );
      }
    }
  ...
  if (!out.Dynasty_SF.length && !out.Dynasty_1QB.length) {
    const sampleSheet = sheets.find(
      (s) => Array.isArray(s.rows) && s.rows.length,
    )?.rows?.[0];
    const sampleKeys =
      sampleSheet && typeof sampleSheet === "object"
  ...
  console.log(
    `✅ idpshow_cache.json updated (value-only). Kept ${totalKept} rows.`,
  );
}
...
async function updateStickyPicky() {
  ...
  for (const source of sources) {
    try {
      ...
      if (source.required) {
        throw new Error(
          `Failed to load required source ${source.name}: ${error.message}`,
        );
      } else {
        console.warn(
          `    ⚠️ ${source.name} failed to load, skipping: ${error.message}`,
        );
      }
    }
  }
  ...
  const {
    ...
    ktc: ktcData,
    fantasynavigator: fnData,
    fantasypros: fpData,
    fantasyprosecr: fpEcrData,
    idynastyp: idpData,
    idpshow: idpshowData,
  } = loadedData;
  ...
  for (const key of [
    ...
    (fcData[key] || []).forEach((row) => {
      ...
      const position = (row.player?.position || row.position || "")
        .replace(/\d+$/, "")
        .trim();
      tables[key].FC[normName(name)] = {
        name,
        value: row.value || 0,
        team,
        position,
  ...
  for (const key of ["Dynasty_SF", "Dynasty_1QB"]) {
    (fpData?.[key] || []).forEach((row) => {
      tables[key].FP[normName(row.name)] = {
        name: row.name,
        value: row.value || 0,
        team: row.team || "",
  ...
  Object.entries(ecrBuckets).forEach(([formatKey, buckets]) => {
    buckets
      .flatMap((bucket) => fpEcrData?.formats?.[bucket] || [])
      .forEach((row) => {
  ...
  (ktcData.Superflex || []).forEach((p) => {
    tables.Dynasty_SF.KTC[normName(p.name)] = {
      name: p.name,
      value: p.value || 0,
      team: p.team || "",
      position: p.position || "",
  ...
  (ktcData.OneQB || []).forEach((p) => {
    tables.Dynasty_1QB.KTC[normName(p.name)] = {
      name: p.name,
      value: p.value || 0,
      team: p.team || "",
      position: p.position || "",
  ...
  for (const formatKey of Object.keys(out)) {
    ...
    for (const S of sourceKeys)
      Object.keys(sources[S]).forEach((nn) => nameSet.add(nn));
    ...
    for (const nn of nameSet) {
      const sourcePcts = sourceKeys
        .map((source) => {
          ...
          return row && Number(row.value) > 0
            ? { source, row, percentile: pctFns[source](row.value) }
            : null;
        })
        .filter(Boolean);
      ...
      if (
        ...
        sourcePcts.some((entry) => entry.source === "FN")
      )
        continue;

      const corroboratingSorted = corroborating
        .map((entry) => entry.percentile)
        .sort((a, b) => a - b);
      const middle = Math.floor(corroboratingSorted.length / 2);
      const corroboratingMedian =
        corroboratingSorted.length % 2
      ...
      const adjusted = sourcePcts.map((entry) => {
        const sourceCenter =
          entry.source === "FN" ? corroboratingMedian : marketMedian;
        const maxDistance = entry.source === "FN" ? 0.12 : 0.18;
        return {
          ...entry,
          adjustedPercentile: Math.max(
            sourceCenter - maxDistance,
            Math.min(sourceCenter + maxDistance, entry.percentile),
          ),
      ...
      const consensusPct =
        adjusted.reduce(
          (sum, entry) => sum + entry.adjustedPercentile * entry.weight,
          0,
        ) / Math.max(0.01, totalWeight);
      const independentCoverage = new Set(
        adjusted.map((entry) =>
          ["IDP", "IDPSHOW"].includes(entry.source)
            ? "IDP_FAMILY"
            : entry.source,
        ),
      ).size;
      ...
      const variance =
        adjusted.reduce(
          (sum, entry) => sum + (entry.adjustedPercentile - consensusPct) ** 2,
          0,
      ...
      out[formatKey].push({
        ...
        confidence:
          independentCoverage >= 4
            ? "high"
            : independentCoverage >= 2
              ? "medium"
              : "low",
        confidence_score: Math.round(
          Math.max(
            20,
            Math.min(98, 45 + independentCoverage * 11 - disagreement * 120),
          ),
        ),
  ...
  out.metadata = {
    ...
    sleeper_context_policy:
      "Sleeper age, experience, status and depth context are retained by the app for explanation and confidence. They do not receive a second arbitrary dynasty-value adjustment because market sources already price those traits.",
  };
...
function pickAnchorSlot(meta) {
  ...
  if (meta?.kind === "bucket")
    return meta.bucket === "early" ? 2 : meta.bucket === "late" ? 11 : 6.5;
  return meta?.kind === "generic" ? 6.5 : 0;
}

function interpolatePickSlot(anchors, slot) {
  const usable = anchors
    .filter((row) => row.slot > 0 && row.value > 0)
    .sort((a, b) => a.slot - b.slot);
  if (!usable.length) return 0;
  if (usable.length === 1)
    return Math.round(usable[0].value * Math.pow(0.96, slot - usable[0].slot));
  let left = usable[0];
  let right = usable[usable.length - 1];
  for (let index = 0; index < usable.length - 1; index += 1) {
    if (slot >= usable[index].slot && slot <= usable[index + 1].slot) {
      left = usable[index];
      right = usable[index + 1];
      break;
    }
    if (slot < usable[0].slot) {
      left = usable[0];
      right = usable[1];
      break;
    }
    if (slot > usable[usable.length - 1].slot) {
      left = usable[usable.length - 2];
      right = usable[usable.length - 1];
    }
  }
  ...
  const raw = Math.exp(
    Math.log(left.value) +
      (Math.log(right.value) - Math.log(left.value)) * progress,
  );
...
function addCalculatedSlotsToArray(rows, valueOf, createRow) {
  ...
  rows.forEach((row) => {
    ...
    const pos = normalizePos(
      row?.player?.position || row?.position || row?.pos,
    );
    if (pos !== "PICK") return;
    ...
    for (let slot = 1; slot <= GENERATED_PICK_SLOTS; slot += 1) {
      ...
      rows.push(
        createRow({
          ...
          slot,
          value,
          method: "source-curve interpolation",
        }),
      );
      added += 1;
    }
  });
  const playerScale = rows
    .filter(
      (row) =>
        normalizePos(row?.player?.position || row?.position || row?.pos) !==
        "PICK",
    )
    .map((row) => Number(valueOf(row)) || 0)
    ...
    playerScale.length
      ? playerScale[
          Math.max(
            0,
            Math.min(
              playerScale.length - 1,
              Math.round((1 - percentile) * (playerScale.length - 1)),
            ),
          )
        ]
      : 0;
    ...
    for (let slot = 1; slot <= GENERATED_PICK_SLOTS; slot += 1) {
      ...
      rows.push(
        createRow({
          ...
          slot,
          value,
          method: "player-scale percentile model",
        }),
      );
      added += 1;
    }
...
function normalizeCalculatedPickSlots(selectedSources) {
  ...
  const updateJson = (key, filePath, mutate) => {
    ...
    total += added;
    console.log(
      `  🧮 ${path.basename(filePath)}: ${added} calculated exact pick values added`,
    );
  };
  const standardRow = ({ year, round, slot, value, method }) => ({
    name: formatPickLabel({ year, round, slot, kind: "exact" }),
    team: "",
    position: "PICK",
    value,
    generatedPickValue: true,
    pickValueMethod: method,
  });
  updateJson("fc", FC_OUT_PATH, (data) =>
    ["Dynasty_SF", "Dynasty_1QB", "Redraft_SF", "Redraft_1QB"].reduce(
      (sum, key) =>
        sum +
        addCalculatedSlotsToArray(
          data[key],
          (row) => row.value,
          ({ year, round, slot, value, method }) => ({
            player: {
              name: formatPickLabel({ year, round, slot, kind: "exact" }),
              sleeperId: `DP_${round - 1}_${slot - 1}`,
              position: "PICK",
              maybeTeam: null,
            },
            value,
  ...
  updateJson("ktc", KTC_OUT_PATH, (data) =>
    ["Superflex", "OneQB"].reduce(
      (sum, key) =>
        sum +
        addCalculatedSlotsToArray(data[key], (row) => row.value, standardRow),
      0,
    ),
  );
  updateJson("fn", FN_OUT_PATH, (data) =>
    ["Dynasty_SF", "Dynasty_1QB", "Redraft_SF", "Redraft_1QB"].reduce(
      (sum, key) =>
        sum +
        addCalculatedSlotsToArray(data[key], (row) => row.value, standardRow),
  ...
  updateJson("sp", SP_OUT_PATH, (data) =>
    ["Dynasty_SF", "Dynasty_1QB", "Redraft_SF", "Redraft_1QB"].reduce(
      (sum, key) =>
        sum +
        addCalculatedSlotsToArray(data[key], (row) => row.value, standardRow),
  ...
  updateJson("idp", IDP_OUT_PATH, (data) => {
    const sf = addCalculatedSlotsToArray(
      data,
      (row) => row.superflex,
      ({ year, round, slot, value, method }) => ({
        name: formatPickLabel({ year, round, slot, kind: "exact" }),
    ...
    const qbRows = data.map((row) => ({ ...row, value: row.one_qb }));
    const generated = [];
    const qb = addCalculatedSlotsToArray(
      qbRows,
      (row) => row.value,
      ({ year, round, slot, value, method }) => ({
        name: formatPickLabel({ year, round, slot, kind: "exact" }),
    ...
    generated.forEach((row) => {
      const target = data.find((item) => item.name === row.name);
      if (target) target.one_qb = row.value;
      else data.push({ ...row, one_qb: row.value, superflex: 0 });
    });
    return sf + qb;
  });
  updateJson("idpshow", IDPSHOW_OUT_PATH, (data) =>
    ["Dynasty_SF", "Dynasty_1QB", "Redraft_SF", "Redraft_1QB"].reduce(
      (sum, key) =>
        sum +
        addCalculatedSlotsToArray(data[key], (row) => row.value, standardRow),
  ...
  updateJson("dp", DP_OUT_PATH, (data) => {
    const rows = Object.entries(data).map(([name, row]) => ({
      name,
      position: row.pos,
      value: row.superflex,
      raw: row,
    ...
    const generated = [];
    const sfAdded = addCalculatedSlotsToArray(
      rows,
      (row) => row.value,
      ({ year, round, slot, value }) => ({
        name: formatPickLabel({ year, round, slot, kind: "exact" }),
        position: "PICK",
        value,
        year,
    ...
    generated.forEach((row) => {
      data[row.name] = {
        ...
        superflex: row.value,
        generatedPickValue: true,
        pickValueMethod: "source-curve interpolation",
      };
    });
    const qbRows = Object.entries(data).map(([name, row]) => ({
      name,
      position: row.pos,
      value: row.one_qb,
      raw: row,
    ...
    const qbGenerated = [];
    const qbAdded = addCalculatedSlotsToArray(
      qbRows,
      (row) => row.value,
      ({ year, round, slot, value }) => ({
        name: formatPickLabel({ year, round, slot, kind: "exact" }),
  ...
  if (total)
    console.log(
      `✅ Added ${total} calculated current-year exact pick values without replacing provider values.`,
    );
}
...
async function fetchCurrentSeasonFromSleeper() {
  try {
    ...
    return Number(
      data?.league_season || data?.season || new Date().getFullYear(),
    );
  } catch {
...
async function scrapeGridironGamesByes(season) {
  ...
  try {
    await page.waitForSelector("#cn-accept-cookie, .cn-set-cookie", {
      timeout: 3000,
    });
    const accept =
      (await page.$("#cn-accept-cookie")) || (await page.$(".cn-set-cookie"));
    if (accept) {
      await accept.click();
      await sleep(500);
    }
...
async function updateByeWeeksAuto() {
  ...
  console.log(
    `✅ Bye weeks written: ${path.relative(process.cwd(), outPath)} (source: ${data.source})`,
  );
}
...
async function updateProjectionsFromCSV() {
  ...
  if (!fs.existsSync(PROJ_IN_PATH)) {
    throw new Error(
      `CSV not found at ${PROJ_IN_PATH}. Pass PROJ_CSV=/path/to/file.csv or move the file to /data.`,
    );
  }
  ...
  const nameKey = keyOf(
    keys.map((k) => k.toLowerCase()).includes("player")
      ? ["player"]
  ...
  if (!nameKey)
    throw new Error(
      "Could not find a name column (expected 'player' or 'name').",
    );
  ...
  for (const r of parsed) {
    ...
    if ((!team || !position) && player_id && sleeperById[player_id]) {
      ...
      position =
        position || (sleeperById[player_id].position || "").toUpperCase();
    }
  ...
  console.log(
    `✅ projections_${CURRENT_SEASON}.json written with ${rows.length} rows.`,
  );
}
...
async function updateESPNProjections() {
  ...
  const API_BASE = `https://lm-api-reads.fantasy.espn.com/apis/v3/games/ffl/seasons/${CURRENT_SEASON}/segments/0/leaguedefaults/3`;
  ...
  function teamFromPlayer(pl) {
    const a =
      pl?.proTeamAbbrev || pl?.teamAbbrev || pl?.proTeam || pl?.team || "";
  ...
  function posFromPlayer(pl) {
    const p = pl?.defaultPosition || pl?.position || pl?.pos || "";
  ...
  for (let page = 1; page <= 1000; page++) {
    ...
    console.log(
      `ESPN API page ${page}: offset=${offset} got=${players.length}`,
    );
  ...
  console.log(
    `✅ projections_espn_${CURRENT_SEASON}.json written (${rows.length} rows).`,
  );
}
...
async function updateSleeperProjections() {
  console.log(
    `Fetching Sleeper ${CURRENT_SEASON} weekly projections (undocumented read-only endpoint)...`,
  );
  ...
  const weekly = await Promise.all(
    weeks.map(async (week) => {
      ...
      const response = await retryOperation(() =>
        axios.get(url, { timeout: 60000, family: 4 }),
      );
      if (
        !response.data ||
        ...
        throw new Error(
          `Sleeper returned an invalid projection payload for Week ${week}`,
        );
      }
      const projected = Object.values(response.data).filter(
        (row) =>
          Number(row?.pts_ppr) > 0 ||
          Number(row?.pts_half_ppr) > 0 ||
          Number(row?.pts_std) > 0,
      ).length;
  ...
  const rows = [...totals.values()]
    .map((row) => {
      ...
      return {
        ...
        name:
          player.name ||
          (isDefense ? `${row.player_id} Defense` : `Player ${row.player_id}`),
  ...
  const missingCore = ["QB", "RB", "WR", "TE"].filter(
    (position) => Number(coreCounts[position] || 0) < 20,
  );
  if (missingCore.length) {
    throw new Error(
      `Sleeper projection coverage is incomplete (${JSON.stringify(coreCounts)}). Existing cache was not overwritten.`,
    );
  }

  const by_id = Object.fromEntries(
    rows.map((row) => [row.player_id, row.points]),
  );
  const by_name = {};
  rows.forEach((row) => {
  ...
  console.log(
    `✅ projections_sleeper_${CURRENT_SEASON}.json written (${rows.length} players).`,
  );
}
...
function projectionOutput(source, rows, extra = {}) {
  const clean = rows
    .filter((row) => row.name && row.position && Number(row.points) > 0)
    .sort((a, b) => b.points - a.points);
  const by_name = {};
  clean.forEach((row) => {
    by_name[normNameForMap(row.name)] = row.points;
...
async function updateFantasyProsProjections() {
  ...
  if (!response.ok)
    throw new Error(
      `FantasyPros projections returned HTTP ${response.status}.`,
    );
  const payload = await response.json();
  ...
  if (declaredCount > sourceRows.length && sourceRows.length <= 10) {
    throw new Error(
      `FantasyPros API access returned only ${sourceRows.length} of ${declaredCount} declared projections. The key works, but its current access tier is returning a sample rather than the complete projection set.`,
    );
  }
  const rows = sourceRows
    .map((player) => {
      ...
      const pointsPpr =
        Number(stats.points_ppr ?? player.points_ppr ?? pointsStd) || pointsStd;
      const pointsHalf =
        Number(
          stats.points_half ??
            player.points_half ??
            (pointsStd + pointsPpr) / 2,
        ) || pointsStd;
      return {
        player_id: "",
    ...
    .filter((row) => row.name && row.position && row.points > 0);
  const coreCount = rows.filter((row) =>
    ["QB", "RB", "WR", "TE"].includes(row.position),
  ).length;
  if (coreCount < 100)
    throw new Error(
      `FantasyPros projection coverage is incomplete (${coreCount} offensive players); existing cache was not overwritten.`,
    );
  ...
  console.log(
    `✅ projections_fantasypros_${CURRENT_SEASON}.json written (${rows.length} players, one API request).`,
  );
}
...
async function updateProjectionConsensus({ includeArsenalModel = false } = {}) {
  const sourceFiles = [
    ...
    { key: "CBS", path: CBS_PROJ_OUT_PATH, weight: 1, formats: ["ppr"] },
    ...
    {
      key: "Sleeper",
      path: SLEEPER_PROJ_OUT_PATH,
      weight: 0.35,
      formats: ["ppr", "half", "std"],
    },
    {
      key: "FantasySharks",
      path: FANTASYSHARKS_PROJ_OUT_PATH,
      weight: 1,
      formats: ["ppr"],
    },
    {
      key: "DraftSharks",
      path: DRAFTSHARKS_PROJ_OUT_PATH,
      weight: 1,
      formats: ["ppr", "half", "std"],
    },
    {
      key: "FantasyPros",
      path: FANTASYPROS_PROJ_OUT_PATH,
      weight: 1,
      formats: ["ppr", "half", "std"],
    },
  ];
  ...
  const resolveSleeperContext = (name, position, team) => {
    const exact = sleeperContext.get(`${normNameForMap(name)}|${position}`);
    if (exact) return exact;
    const surname = normNameForMap(name).split(" ").at(-1);
    const candidates = sleeperByTeamPositionSurname.get(
      `${normalizeFantasyTeamAbbr(team)}|${position}|${surname}`,
    );
    // This safely joins common legal-name/nickname differences (Kenny vs
    // Kenneth, Chig vs Chigoziem) only when the team/position/surname identity
    // is unique. Ambiguous names remain separate instead of being guessed.
  ...
  available.forEach((source) => {
    ...
    (Array.isArray(data?.rows) ? data.rows : []).forEach((row) => {
      ...
      const sourceTeam = normalizeFantasyTeamAbbr(row?.team || "");
      const resolved = resolveSleeperContext(name, position, sourceTeam);
      const key = resolved?.player_id
        ? `sleeper:${resolved.player_id}`
        : `${normNameForMap(name)}|${position}`;
      if (!players.has(key))
        players.set(key, {
          name: resolved?.full_name || resolved?.name || name,
          position,
          team: normalizeFantasyTeamAbbr(resolved?.team || sourceTeam),
          context: resolved || null,
          inputs: [],
        });
      const player = players.get(key);
      if (!player.team && row?.team)
        player.team = normalizeFantasyTeamAbbr(row.team);
  ...
  const rows = [...players.values()]
    ...
    .map((player) => {
      const context =
        ...
        {};
      const injury = String(
        context?.injury_status || context?.status || "",
      ).toLowerCase();
      ...
      const sourcePoints = externalInputs
        ...
        .filter((value) => value > 0);
      const average =
        sourcePoints.reduce((sum, value) => sum + value, 0) /
        Math.max(1, sourcePoints.length);
      const disagreement =
        average > 0
          ? Math.sqrt(
              sourcePoints.reduce(
      ...
      if ((externalInputs.length < 4 || disagreement > 0.22) && depth) {
        ...
        contextReasons.push(
          `Depth-chart order ${depth} used as a bounded correction`,
        );
      }
      ...
      const confidence = Math.round(
        Math.max(
          ...
          Math.min(
            98,
            42 +
              player.inputs.length * 8 -
              disagreement * 70 -
              (contextReasons.length ? 5 : 0),
          ),
        ),
      );
      return {
        ...
        format_source_count: {
          ppr: player.inputs.filter(
            (input) => Number.isFinite(input.ppr) && input.ppr > 0,
          ).length,
          half: player.inputs.filter(
            (input) => Number.isFinite(input.half) && input.half > 0,
          ).length,
          std: player.inputs.filter(
            (input) => Number.isFinite(input.std) && input.std > 0,
          ).length,
        },
        confidence,
        disagreement: Number(disagreement.toFixed(4)),
        ...
        sources: Object.fromEntries(
          player.inputs.map((input) => [
            input.source,
            {
              ...
              half: Number.isFinite(input.half)
                ? Number(input.half.toFixed(3))
                : null,
              std: Number.isFinite(input.std)
                ? Number(input.std.toFixed(3))
                : null,
            },
          ]),
        ),
  ...
  const output = projectionOutput(
    ...
    format_policy: {
      ...
      source_formats: Object.fromEntries(
        sourceFiles.map((source) => [source.key, source.formats]),
      ),
    },
    context_policy: {
      direct_inputs: [
        ...
        "Sleeper depth-chart order when coverage is thin or disagreement is high",
      ],
      confidence_inputs: [
        "source count",
        "source disagreement",
...
async function updateFantasySharksProjections() {
  ...
  try {
    ...
    try {
      ...
      if (Number(cached?.season) === CURRENT_SEASON && cached?.segment) {
        ...
        console.warn(
          `⚠️ ${error.message}; retrying the last verified FantasySharks season segment (${segment}).`,
        );
      } else {
        throw error;
      }
  ...
  if (parsed.errors?.length && !parsed.data?.length)
    throw new Error(
      `FantasySharks CSV parse failed: ${parsed.errors[0].message}`,
    );
  const rows = parsed.data.map((row) => {
    ...
    const parts = raw
      ...
      .filter(Boolean);
    const name =
      parts.length > 1 ? `${parts.slice(1).join(" ")} ${parts[0]}` : raw;
    return {
      player_id: "",
      ...
      stats: {
        pass_yds: Number(row["Pass Yds"]) || 0,
        pass_tds: Number(row["Pass TDs"]) || 0,
        rush_yds: Number(row["Rush Yds"]) || 0,
        rush_tds: Number(row["Rush TDs"]) || 0,
        receptions: Number(row.Rec) || 0,
        rec_yds: Number(row["Rec Yds"]) || 0,
        rec_tds: Number(row["Rec TDs"]) || 0,
      },
    };
  });
  if (
    rows.filter(
      (row) =>
        ["QB", "RB", "WR", "TE"].includes(row.position) && row.points > 0,
    ).length < 100
  )
    throw new Error(
      "FantasySharks projection coverage is incomplete; existing cache was not overwritten.",
    );
  fs.writeFileSync(
    FANTASYSHARKS_PROJ_OUT_PATH,
    JSON.stringify(
      projectionOutput("FantasySharks", rows, { endpoint: url, segment }),
      null,
      2,
    ),
  );
  console.log(
    `✅ projections_fantasysharks_${CURRENT_SEASON}.json written (${rows.length} rows).`,
  );
}
...
function parseDraftSharksRows(html) {
  ...
  const blocks =
    String(html || "").match(
      /<tbody\b[^>]*\bdata-player-row\b[\s\S]*?<\/tbody>/gi,
    ) || [];
  blocks.forEach((block, index) => {
    const attr = (name) =>
      decodeHtml(block.match(new RegExp(`${name}="([^"]*)"`, "i"))?.[1] || "");
    const cells = {};
    for (const match of block.matchAll(
      /<td[^>]*data-value="([^"]*)"[^>]*data-attribute="([^"]*)"[^>]*>([\s\S]*?)<\/td>/gi,
    ))
      cells[decodeHtml(match[2])] = Number(match[1]) || stripHtml(match[3]);
    const analysis = {};
    const analysis_links = {};
    for (const match of block.matchAll(
      /<td[^>]*data-attribute="([^"]*(?:analysis|bottom|risk|upside|floor|ceiling)[^"]*)"[^>]*>([\s\S]*?)<\/td>/gi,
    )) {
      const key = decodeHtml(match[1]);
      analysis[key] = stripHtml(match[2]);
      const href = decodeHtml(
        match[2].match(
          /<a[^>]*class="[^"]*bottom-line-profile-link[^"]*"[^>]*href="([^"]+)"/i,
        )?.[1] ||
          match[2].match(
            /<a[^>]*href="([^"]+)"[^>]*>\s*(?:Read Full Profile|Click to Read)/i,
          )?.[1] ||
          "",
      );
      if (href)
        analysis_links[key] = new URL(href, "https://www.draftsharks.com").href;
    }
    rows.push({
      player_id: "",
      source_player_id: attr("data-key"),
      name: attr("data-player-name"),
      team: normalizeFantasyTeamAbbr(
        stripHtml(
          block.match(
            /player-details-group__team-name[^>]*>([\s\S]*?)<\/span>/i,
          )?.[1] || "",
        ),
      ),
      position: normalizePos(attr("data-fantasy-position")),
      points: Number(cells.weekly3dPts) || 0,
      source_rank: index + 1,
      projections: cells,
      analysis,
      analysis_links,
...
async function updateDraftSharksProjections() {
  ...
  const query = (depth, slug = "") =>
    `${base}?pprSuperflexSlug=${slug}&fantasyPosition=&researchDepth=${depth}&playerGroup=all&sort=-dsValue&selectedTeam=&playerSearchTerm=`;
  const variants = [
    { key: "std", slug: "" },
    { key: "half", slug: "half-ppr" },
    { key: "ppr", slug: "ppr" },
    { key: "tep", slug: "te-premium" },
  ];
  const fetchHtml = (url, label) =>
    fetch(url, {
      headers: { "user-agent": "Mozilla/5.0", accept: "text/html" },
    }).then((response) => {
      if (!response.ok)
        throw new Error(
          `DraftSharks ${label} endpoint returned HTTP ${response.status}`,
        );
      return response.text();
    });
    ...
    Promise.all(
      variants.map(async (variant) => ({
        ...
        html: await fetchHtml(
          query("projections", variant.slug),
          `projections ${variant.key}`,
        ),
      })),
    ),
    fetchHtml(query("analysis"), "analysis"),
  ]);
  const variantRows = Object.fromEntries(
    variantPayloads.map((variant) => [
      variant.key,
      parseDraftSharksRows(variant.html),
    ]),
  );
  const rows = variantRows.ppr.length ? variantRows.ppr : variantRows.std;
  const byVariant = Object.fromEntries(
    Object.entries(variantRows).map(([key, list]) => [
      key,
      new Map(list.map((row) => [row.source_player_id, row])),
    ]),
  );
  rows.forEach((row) => {
    row.points_std =
      Number(byVariant.std.get(row.source_player_id)?.points) || 0;
    row.points_half =
      Number(byVariant.half.get(row.source_player_id)?.points) || 0;
    row.points_ppr =
      Number(byVariant.ppr.get(row.source_player_id)?.points) || row.points;
    row.points_tep =
      Number(byVariant.tep.get(row.source_player_id)?.points) || row.points_ppr;
  });
  const analysisRows = new Map(
    parseDraftSharksRows(analysisHtml).map((row) => [
  ...
  rows.forEach((row) => {
    ...
    row.analysis_links =
      analysisRow?.analysis_links || row.analysis_links || {};
    row.analysis_articles = Object.entries(row.analysis).map(([key, text]) => ({
      key,
      text,
      url: row.analysis_links[key] || null,
    }));
  });
  const coreRows = rows.filter(
    (row) => ["QB", "RB", "WR", "TE"].includes(row.position) && row.points > 0,
  );
  const coverage = rows.reduce((map, row) => {
    map[row.position || "UNKNOWN"] =
      (map[row.position || "UNKNOWN"] || 0) + (row.points > 0 ? 1 : 0);
    return map;
  }, {});
  if (coreRows.length < 50)
    throw new Error(
      `DraftSharks projection coverage is incomplete (${JSON.stringify(coverage)}); existing cache was not overwritten.`,
    );
  fs.writeFileSync(
    DRAFTSHARKS_PROJ_OUT_PATH,
    JSON.stringify(
      projectionOutput("DraftSharks", rows, {
        scoring: "STD/HALF/PPR/TEP",
        scoring_variants: ["std", "half", "ppr", "tep"],
        default_scoring: "ppr",
  ...
  console.log(
    `✅ projections_draftsharks_${CURRENT_SEASON}.json written (${rows.length} rows).`,
  );
}
...
async function fetchCBSProjectionRows(position, url) {
  const candidates = [
    ...
    url.replace(
      ...
      "https://secure-www.cbssports.com",
    ),
  ];
  let html = "";
  let lastError = null;
    ...
    try {
      html = await retryOperation(
        async () => {
          ...
          if (!response.ok)
            throw new Error(`CBS ${position} returned HTTP ${response.status}`);
          return response.text();
        },
        2,
        1200,
  ...
  const table = [...html.matchAll(/<table\b[\s\S]*?<\/table>/gi)]
    .map((match) => match[0])
    .find(
      (value) => /TableBase-bodyTr/i.test(value) && />\s*fpts\s*</i.test(value),
    );
  if (!table)
    throw new Error("CBS projection table was not present in the response.");
  const header = table.match(/<thead\b[\s\S]*?<\/thead>/i)?.[0] || "";
  const headerRows = [...header.matchAll(/<tr\b[\s\S]*?<\/tr>/gi)].map(
    (match) => match[0],
  );
  const columnHeader = headerRows[headerRows.length - 1] || header;
  const headers = [...columnHeader.matchAll(/<th\b[\s\S]*?<\/th>/gi)].map(
    (match) => decodeCBSHtml(match[0]).toLowerCase(),
  );
  ...
  for (const match of table.matchAll(
    /<tr\b[^>]*TableBase-bodyTr[^>]*>([\s\S]*?)<\/tr>/gi,
  )) {
    ...
    const longBlock =
      playerCell.match(
        /CellPlayerName--long[\s\S]*?<\/span>\s*<\/span>/i,
      )?.[0] || playerCell;
    const name = decodeCBSHtml(
      longBlock.match(/<a\b[^>]*>([\s\S]*?)<\/a>/i)?.[1],
    );
    const team = normalizeTeamAbbr(
      decodeCBSHtml(
        longBlock.match(/CellPlayerName-team[^>]*>([\s\S]*?)<\/span>/i)?.[1],
      ),
    );
    const parsedPosition = normalizePos(
      decodeCBSHtml(
        longBlock.match(
          /CellPlayerName-position[^>]*>([\s\S]*?)<\/span>/i,
        )?.[1],
      ) || position,
    );
    const points =
...
async function updateCBSProjections() {
  ...
  const missingDirectCore = ["QB", "RB", "WR", "TE"].filter(
    (position) => (directCounts[position] || 0) < 10,
  );
  if (missingDirectCore.length)
    throw new Error(
      `CBS returned incomplete projections (${
    ...
    try {
      ...
      await page.evaluate(() => {
        ...
        const hit = btns.find((b) =>
          /accept|agree|consent|got it|continue/i.test(b.textContent || ""),
        );
        if (hit) hit.click();
      });
  ...
  async function scrapePos(posKey, cfg) {
    ...
    await page.waitForSelector("#TableBase table, main table", {
      timeout: 15000,
    });
      ...
      (posKey, ptsTdIndex1, TEAM_FULL_TO_ABBR) => {
        ...
        function normPos(p) {
          const t = String(p || "")
            .toUpperCase()
            .trim();
          if (t === "DST" || t === "D/ST" || t === "DEFENSE" || t === "DEF")
            return "DEF";
          if (t === "PK") return "K";
          return t;
        }
        ...
        const root =
          document.querySelector("#TableBase table") ||
          (document.querySelector("main") || document.body).querySelector(
            "table",
          );
        if (!root) return [];
        ...
        const headerCells = Array.from(
          root.querySelectorAll("thead tr:last-child th"),
        );
        const detectedPointsIndex = headerCells.findIndex((cell) => {
          const label =
            `${cell.getAttribute("data-label") || ""} ${cell.getAttribute("aria-label") || ""} ${cell.getAttribute("title") || ""} ${text(cell)}`
              .toLowerCase()
              .replace(/[^a-z]/g, " ")
              .replace(/\s+/g, " ")
              .trim();
          return (
            ...
            (label.includes("fantasy points") && !label.includes("per game"))
          );
        });
        ...
        for (const tr of trs) {
          ...
          if (posKey !== "DEF") {
            ...
            const teamTok = tokenTexts.find(
              (tok) => tok !== pos && /^[A-Z]{2,3}$/.test(tok),
            );
          ...
          const labeledPointsIndex = tds.findIndex((cell) => {
            const label =
              `${cell.getAttribute("data-label") || ""} ${cell.getAttribute("aria-label") || ""}`.toLowerCase();
            return (
              (label.includes("fpts") || label.includes("fantasy points")) &&
              !label.includes("per game") &&
              !label.includes("fppg")
            );
          });
          const idx0 =
            labeledPointsIndex >= 0
              ? labeledPointsIndex
              : detectedPointsIndex >= 0
                ? detectedPointsIndex
                : Math.max(1, ptsTdIndex1) - 1;
  ...
  const positionCounts = allRows.reduce(
    (counts, row) => ({
      ...counts,
      [row.position]: (counts[row.position] || 0) + 1,
    }),
  ...
  const missingCore = ["QB", "RB", "WR", "TE"].filter(
    (position) => (positionCounts[position] || 0) < 10,
  );
  if (missingCore.length) {
    if (!KEEP_OPEN) await browser.close();
    throw new Error(
      `CBS returned an incomplete projection table (${
  ...
  console.log(
    `✅ projections_cbs_${CURRENT_SEASON}.json written (${rows.length} rows).`,
  );
}
...
(async () => {
  try {
    ...
    if (process.argv.includes("--normalize-picks-only")) {
      console.log(
        "🧮 Normalizing current-year pick slots in existing value caches...",
      );
      normalizeCalculatedPickSlots([
        "fc",
        "dp",
        "ktc",
        "fn",
      ...
      : await inquirer.prompt([
          {
            ...
            choices: [
              ...
              {
                name: "FantasyPros (official public dynasty charts)",
                value: "fp",
              },
              {
                name: "FantasyPros ECR Rank Score (8 unique official API ranking boards)",
                value: "fantasypros_ecr",
              },
              ...
              {
                name: "Sleeper Projections (undocumented API)",
                value: "sleeper_proj",
              },
              {
                name: "FantasySharks Projections (first-party CSV)",
                value: "fantasysharks_proj",
              },
              {
                name: "DraftSharks Projections + Analysis (first-party table feed)",
                value: "draftsharks_proj",
              },
              {
                name: "FantasyPros Projections (official API · STD/Half/PPR)",
            ...
            validate: (input) =>
              input.length === 0 ? "Please select at least one." : true,
          },
        ]);
    ...
    const updateTasks = [
      ...
      {
        key: "fantasypros_ecr",
        name: "FantasyPros ECR Rank Score",
        fn: updateFantasyProsECR,
      },
      ...
      { key: "proj", name: "Projections", fn: updateProjectionsFromCSV },
      {
        ...
        fn: updateSleeperProjections,
      },
      {
        key: "fantasysharks_proj",
        name: "FantasySharks Projections",
        fn: updateFantasySharksProjections,
      },
      {
        key: "draftsharks_proj",
        name: "DraftSharks Projections",
        fn: updateDraftSharksProjections,
      },
      {
        key: "fantasypros_proj",
        name: "FantasyPros Projections",
        fn: updateFantasyProsProjections,
      },
    ...
    if (
      ...
      if (failed.length && !process.argv.includes("--daily")) {
        throw new Error(
          `Archive skipped because these updates failed: ${failed.join(", ")}`,
        );
      }
      if (completed === 0)
        throw new Error(
          "Archive skipped because no sources updated successfully.",
        );
    ...
    if (failed.length > 0) {
      ...
      console.log(
        `   💡 Some sources failed but others completed successfully.`,
      );
    } else {
