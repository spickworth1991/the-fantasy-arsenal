...
async function sleeper(season) {
  ...
  return {
    ...
    season,
    updated:new Date().toISOString(),
    completed_weeks:weeks.filter((week) => Object.keys(week.rows).length).length,
    count:players.length,
    players,
  };
}

const manifestPath = path.join(root, "public", "stats", "history", "manifest.json");
function saveManifestEntry(entry) {
  let current = { seasons:[] };
  try { current = JSON.parse(fs.readFileSync(manifestPath, "utf8")); } catch {}
  const rows = [entry, ...(current.seasons || []).filter((row) => Number(row.season) !== Number(entry.season))]
...
for (const season of seasons) {
  ...
  Object.entries(scoringPayloads).forEach(([scoring, payload]) => payload.players.forEach((player) => {
    const key = String(player.player_id || `${player.name}|${player.position}`);
    const current = identities.get(key) || {
      player_id:player.player_id, name:player.name, position:player.position, team:player.team, scoring:{},
    };
    current.scoring[scoring.toLowerCase()] = {
      games:player.games, points:player.points, average:player.average, weeks:player.weeks,
    };
  ...
  for (const legacy of ["fantasypros_std.json","fantasypros_half.json","fantasypros_ppr.json"]) {
