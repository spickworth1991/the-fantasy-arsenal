...
if (missingLeaderboardLeagues.length) {
  console.warn(`Ballsville draft feeds are incomplete: ${missingLeaderboardLeagues.length} leaderboard league${missingLeaderboardLeagues.length === 1 ? " is" : "s are"} missing. Publishing all available statistics with a coverage warning.`);
}
const joinResults = await mapLimit(draftRows, 6, async ({mode,league}) => {
  try {
    ...
    return {ok:true,data:{mode,league,leagueInfo,users,rosters,picks:picks.map((pick)=>({...pick,ownerId:clean(pick?.picked_by||rosterOwner.get(clean(pick?.roster_id))) }))}};
  } catch (error) {
    console.warn(`Skipping ${league?.leagueId || "unknown league"} (${mode?.title || mode?.modeSlug}): ${error.message}`);
    return {ok:false,mode,league,error:clean(error?.message)};
  }
});
const joined = joinResults.filter((row)=>row?.ok&&row?.data).map((row)=>row.data);
const unavailableDraftRows = joinResults.filter((row)=>row&&!row.ok).map(({mode,league,error})=>({mode:mode?.modeSlug,name:clean(league?.leagueName||league?.name||league?.leagueId),leagueId:clean(league?.leagueId),draftId:clean(league?.draftId),reason:error||"Sleeper data unavailable"}));
if (unavailableDraftRows.length) console.warn(`Publishing without ${unavailableDraftRows.length} draft row${unavailableDraftRows.length === 1 ? "" : "s"} whose Sleeper data was unavailable.`);
if (!joined.length) throw new Error("No Ballsville draft rows could be joined to Sleeper; refusing to replace the statistics cache with an empty file.");
...
const missingCoverageRows=[...missingLeaderboardLeagues,...unavailableDraftRows].filter((row,index,all)=>index===all.findIndex((candidate)=>candidate.leagueId===row.leagueId&&clean(candidate.draftId)===clean(row.draftId)));
const output={schemaVersion:5,season,previousSeason:season-1,generatedAt:new Date().toISOString(),source:{ballsville:"Published draft-compare mode boards",sleeper:"League users, rosters, and draft picks",leaderboards:"Published Ballsville leaderboard league directory"},coverage:{complete:missingCoverageRows.length===0,leaderboardLeagues:new Set(leaderboardLeagues.map((row)=>row.leagueId)).size,includedLeagues:includedLeagueIds.size,missingLeagues:missingCoverageRows,missingDraftFeedLeagues:missingLeaderboardLeagues,unavailableSleeperDrafts:unavailableDraftRows},summary:{totalOwners:allOwners.size,totalSeats:joined.reduce((sum,row)=>sum+num(row.league?.meta?.teams),0),uniqueRosterSeats:uniqueRosterSeats.size,totalLeagues:includedLeagueIds.size,totalDrafts:new Set(joined.map(row=>clean(row.league.draftId))).size,totalModes:modeRows.length,totalPlayers:playerRows.length,crossModeOwners:[...ownerModes.values()].filter((set)=>set.size>1).length},modes:modeRows,players:playerRows,teams:teamRows};
