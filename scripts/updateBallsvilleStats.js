...
joined.forEach(({mode,league,users,picks})=>{const stat=modeStats.get(mode.modeSlug);stat.leagueIds.add(clean(league.leagueId));stat.draftIds.add(clean(league.draftId));stat.seats+=num(league?.meta?.teams);
  users.forEach((user)=>{const id=clean(user?.user_id);if(!id)return;ownersById.set(id,publicOwner(user));stat.ownerIds.add(id);allOwners.add(id);if(!ownerModes.has(id))ownerModes.set(id,new Set());ownerModes.get(id).add(mode.modeSlug);});
...
const output={schemaVersion:3,season,previousSeason:season-1,generatedAt:new Date().toISOString(),source:{ballsville:"Published draft-compare mode boards",sleeper:"League users, rosters, and draft picks"},summary:{totalOwners:allOwners.size,totalSeats:joined.reduce((sum,row)=>sum+num(row.league?.meta?.teams),0),totalDrafts:new Set(joined.map(row=>clean(row.league.draftId))).size,totalModes:modeRows.length,totalPlayers:playerRows.length,crossModeOwners:[...ownerModes.values()].filter((set)=>set.size>1).length},modes:modeRows,players:playerRows,teams:teamRows};
