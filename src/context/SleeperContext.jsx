...
export const SleeperProvider = ({ children }) => {
  ...
  try {
    ...
    Object.keys(playersData).forEach((id) => {
      ...
      const idp_values =
      idpCandidate ? { one_qb: idpCandidate.one_qb, superflex: idpCandidate.superflex }
      ...
      const ktc_values = ktcCandidate && ["QB", "RB", "WR", "TE"].includes(pos) ? ktcCandidate : { one_qb: 0, superflex: 0 };
    ...
    Object.keys(dpData).forEach((name) => {
      ...
      const matchedSleeper = Object.values(playersData).some((p) => p.full_name && normalizeName(p.full_name) === normName);
