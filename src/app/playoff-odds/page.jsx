...
import { PROJECTION_DATA_SEASON, PROJ_ARSENAL_JSON_URL, PROJ_ARSENAL_MODEL_JSON_URL, PROJ_CBS_JSON_URL, PROJ_DRAFTSHARKS_JSON_URL, PROJ_ESPN_JSON_URL, PROJ_FANTASYSHARKS_JSON_URL, PROJ_JSON_URL, PROJ_SLEEPER_JSON_URL } from "../../lib/projectionSeason";
...
function ScenarioWorkbench({ scenarioData, results, league, activeLeague, focusRosterId, onImportTrade }) {
...
export default function PlayoffOddsPage() {
    ...
    (async () => {
      ...
      try {
        ...
        if (!next.CSV && !next.ESPN && !next.CBS && !next.SLEEPER && !next.FANTASYSHARKS && !next.DRAFTSHARKS && !next.ARSENAL && !next.ARSENAL_MODEL) {
          ...
          setSourceKey(next.ESPN ? "proj:espn" : next.CSV ? "proj:ffa" : "proj:cbs");
        } else if (projectionSource === "ARSENAL" && !next.ARSENAL) {
          setSourceKey(next.CSV ? "proj:ffa" : next.ESPN ? "proj:espn" : "proj:cbs");
        } else if (projectionSource === "ARSENAL_MODEL" && !next.ARSENAL_MODEL) {
  ...
  const importAnalyzerTrade = () => {
    try {
      const raw = localStorage.getItem(`tfa:trade-handoff:${activeLeague}`);
      if (!raw) return "Build a trade for this league in the Trade Analyzer first.";
      const trade = JSON.parse(raw);
      const focusOwnerId = String(focusRoster?.owner_id || "");
      const isSideA = focusOwnerId && focusOwnerId === String(trade.ownerA || "");
      const isSideB = focusOwnerId && focusOwnerId === String(trade.ownerB || "");
      if (!isSideA && !isSideB) return "The saved trade does not include the selected playoff team.";
      const give = isSideA ? trade.sideB : trade.sideA;
      const receive = isSideA ? trade.sideA : trade.sideB;
      const giveId = (give || []).find((id) => focusTradeAssets.some((player) => String(player.player_id) === String(id)));
      const receiveId = (receive || []).find((id) => receiveTradeAssets.some((row) => String(row.player.player_id) === String(id)));
      if (!giveId || !receiveId) return "This impact model currently requires at least one player sent and one player received.";
      setTradeGiveId(String(giveId));
      setTradeReceiveId(String(receiveId));
      return (give.length > 1 || receive.length > 1)
        ? "Imported the first player swap from the package; multi-player simulation is not yet supported."
        : "Trade imported into Proposed Trade Impact.";
    } catch {
      return "The saved Analyzer trade could not be read.";
    }
  };
                ...
                <Card className="overflow-hidden border-violet-300/15">
                  ...
                  {scenarioOpen && scenarioData ? <div className="border-t border-white/10 p-4 sm:p-6">
                    {results.futureWeeks.length ? <>
                      ...
                      <ScenarioWorkbench scenarioData={scenarioData} results={results} league={league} activeLeague={activeLeague} focusRosterId={focusRosterId} onImportTrade={importAnalyzerTrade}/>
