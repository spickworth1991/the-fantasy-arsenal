...
export default function PlayoffOddsPage() {
  ...
  const getValue = useMemo(() => makeGetPlayerValue(valueSource, format, qbType), [valueSource, format, qbType]);
  ...
  const loadWeek = async (w) => {
    ...
    const pairs = [];
    for (const [,arr] of byMid) if (arr.length === 2) pairs.push({ ridA: arr[0].roster_id, ridB: arr[1].roster_id });
    setSchedCache(s => ({ ...s, [w]: pairs }));
    return pairs;
  };
  ...
  const simulate = async () => {
    ...
    try {
      ...
      const pairsByWeek = await Promise.all(weeks.map(loadWeek));
      ...
      const ridList = rosters.map(r => r.roster_id);

      // strength map per week
      const strengthsByWeek = {};
      weeks.forEach(w => {
        strengthsByWeek[w] = Object.fromEntries(rosters.map(r => [
          r.roster_id,
          teamStrength({ roster: r, players, getValue, slots, week: w, byeMap })
        ]));
      });
      ...
      for (let run = 0; run < runs; run++) {
        ...
        weeks.forEach((w, idx) => {
          (pairsByWeek[idx] || []).forEach(({ ridA, ridB }) => {
            const sA = strengthsByWeek[w][ridA] || 1;
            const sB = strengthsByWeek[w][ridB] || 1;
            const scale = Math.max(50, (sA + sB) / 10);
            const pA = 1 / (1 + Math.exp(-(sA - sB) / scale));
            (Math.random() < pA) ? (wins[ridA] += 1) : (wins[ridB] += 1);
          });
        });
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeLeague, week, toWeek, runs, valueSource, format, qbType, players, rosters.length, byeMap]);
      ...
      <div className="max-w-7xl mx-auto px-4 pt-20 pb-10">
        <Card className="p-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <input type="number" min={1} max={18} value={week} onChange={e=>setWeek(parseInt(e.target.value||"1",10))}
              className="bg-gray-800 text-white p-2 rounded w-20" />
            <span>to</span>
            <input type="number" min={week} max={18} value={toWeek} onChange={e=>setToWeek(parseInt(e.target.value||"18",10))}
              className="bg-gray-800 text-white p-2 rounded w-20" />

            <span className="font-semibold ml-2">Runs:</span>
            <input type="number" min={200} max={10000} step={100} value={runs} onChange={e=>setRuns(parseInt(e.target.value||"2000",10))}
