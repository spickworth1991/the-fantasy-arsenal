...
export default function ClientResults({ initialSearchParams = {} }) {
  ...
  // helper to read server-provided params (strings or arrays)
  const getParam = (k) => {
    const v = initialSearchParams?.[k];
    return Array.isArray(v) ? v[0] : v ?? null;
  };
  // stable key for effect deps when params change
  const paramsKey = JSON.stringify({
    year: getParam("year"),
    only_bestball: getParam("only_bestball"),
    exclude_bestball: getParam("exclude_bestball"),
    include_drafting: getParam("include_drafting"),
  ...
  useEffect(() => {
    ...
    const onlyBB = getParam("only_bestball") === "1";
    const excludeBB = getParam("exclude_bestball") === "1";
    const includeDrafting = getParam("include_drafting") !== "0"; // default include
    const force = getParam("force") === "1";
    ...
    return () => { cancel = true; };
  }, [username, year, paramsKey]); // ⬅️ no `useSearchParams` dep
            ...
            <div className="bg-gray-900 rounded-lg p-4 mb-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="text-sm text-gray-400">
                  ...
                  {lastUpdated && (
                    <span className="ml-3 text-xs text-gray-500" suppressHydrationWarning>Last scan: {lastUpdated.toLocaleTimeString()}</span>
                  )}
