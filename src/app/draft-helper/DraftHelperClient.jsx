...
function DraftIntelligence({ nextMyPick, picksAway, recentRun, tierSize, valueCliff, draftedCounts, byeSummary, topNeed }) {
  return <Panel className="mt-5 overflow-hidden"><div className="border-b border-white/10 p-5"><div className="text-[10px] font-semibold uppercase tracking-[.22em] text-violet-200/55">Live decision dashboard</div><h3 className="mt-1 text-xl font-black">What matters before your next pick</h3><p className="mt-1 text-xs text-white/35">Board pressure, roster construction, and schedule risk in one place.</p></div><div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3 xl:grid-cols-8"><Stat label="Your next pick" value={nextMyPick ? `#${nextMyPick}` : "None"} detail={picksAway != null ? `${picksAway} picks away` : "No remaining owned pick"} /><Stat label="Top roster need" value={topNeed?.pos || "Depth"} detail={topNeed?.gap > 0 ? `${topNeed.gap.toFixed(1)} starter gap` : "Starting requirements covered"} /><Stat label="Bye concentration" value={byeSummary?.week ? `Week ${byeSummary.week}` : "Balanced"} detail={byeSummary?.week ? `${byeSummary.count} players · ${byeSummary.teams.join(", ")}` : "No concentrated bye detected"} /><Stat label="Recent run" value={recentRun?.pos || "None"} detail={recentRun ? `${recentRun.count} of the last ${recentRun.sample} picks` : "Waiting for selections"} /><Stat label="Top value tier" value={tierSize} detail="Players within 10% of BPA" /><Stat label="Value cliff" value={`${valueCliff}%`} detail="BPA to player 12" /><Stat label="Most drafted" value={draftedCounts[0]?.pos || "—"} detail={draftedCounts[0] ? `${draftedCounts[0].count} selected` : "No picks yet"} /><Stat label="Board pressure" value={picksAway != null && picksAway > tierSize ? "Act now" : "Flexible"} detail={picksAway != null && picksAway > tierSize ? "Current tier may not reach you" : "A top tier may survive"} /></div></Panel>;
}
