...
function PlayerSourceDrawer({ item, valueFormat, qbType, getPlayerValue, sourceKey, onClose }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-end justify-center bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:p-4" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><div role="dialog" aria-modal="true" aria-label={`All sources for ${item.name}`} className="max-h-[92vh] w-full overflow-y-auto rounded-t-[30px] border border-white/12 bg-slate-950 shadow-2xl sm:max-w-2xl sm:rounded-[30px]"><div className="sticky top-0 z-10 flex items-center gap-3 border-b border-white/10 bg-slate-950/95 p-4 backdrop-blur"><AvatarImage name={item.name} playerId={item.id} size={46} className="rounded-2xl" alt="" /><div className="min-w-0 flex-1"><div className="truncate text-xl font-black">{item.name}</div><div className="text-xs text-white/38">{item.pos} · {item.team||"FA"} · {valueFormat} · {qbType==="sf"?"Superflex":"1QB"}</div></div><button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-xl bg-white/[0.06] text-lg text-white/60" aria-label="Close source comparison">×</button></div><div className="space-y-5 p-4 sm:p-5"><div className="rounded-2xl border border-cyan-300/10 bg-cyan-300/[0.04] p-3 text-xs leading-5 text-white/45">Values and projections use different scales, so each group has its own consensus. Missing results remain visible instead of silently becoming zero.</div>{section("Player values",values,"")}{section("Season projections",projections," pts")}</div></div></div>;
}
...
export default function DraftHelperClient() {
  ...
  const inspectPlayer = useCallback((input) => {
    ...
    if(!id||!player)return;
    setSourcePlayer(typeof input==="object"&&input?.player ? input : {id,player,name:playerName(player,id),pos:effectivePosition(player),team:player.team,age:n(player.age),value:n(getPlayerValue(player,{format:valueFormat,qbType}))});
  },[getPlayerValue,players,qbType,valueFormat]);
  ...
  return <main className="min-h-screen text-white"><BackgroundParticles /><Navbar pageTitle="Draft Command Center" /><div className="mx-auto max-w-[1500px] px-4 pb-20 pt-20">
    ...
    <PlayerSourceDrawer item={sourcePlayer} valueFormat={valueFormat} qbType={qbType} getPlayerValue={getPlayerValue} sourceKey={sourceKey} onClose={()=>setSourcePlayer(null)} />
  </div></main>;
}
