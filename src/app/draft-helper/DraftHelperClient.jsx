...
function injuryContext(player, valueFormat) {
  ...
  if (["NA", "N/A", "NOT ACTIVE", "INACTIVE"].includes(status)) return { label:label || "NA", multiplier:dynasty ? 0.35 : 0.15, severity:"unavailable" };
...
function DraftIntelligence({ nextMyPick, picksAway, recentRun, tierSize, valueCliff, draftedCounts, byeSummary, topNeed }) {
  const pressure = picksAway != null && picksAway > tierSize ? "Current value tier may not reach you" : "You can stay flexible";
  return <Panel className="mt-5 p-4 sm:p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="text-[10px] font-semibold uppercase tracking-[.22em] text-violet-200/55">Before your next pick</div><h3 className="mt-1 text-lg font-black">{nextMyPick ? `Pick #${nextMyPick} · ${picksAway} picks away` : "No remaining owned pick"}</h3></div><div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-white/45"><span><b className="text-white/75">Need:</b> {topNeed?.pos || "Depth"}</span><span><b className="text-white/75">Recent run:</b> {recentRun ? `${recentRun.pos} (${recentRun.count}/${recentRun.sample})` : "None"}</span><span className={picksAway != null && picksAway > tierSize ? "text-amber-100" : "text-emerald-100/70"}>{pressure}</span></div></div></Panel>;
}
...
export default function DraftHelperClient() {
  ...
  const selectLeague = useCallback((leagueId) => {
    ...
    routeLeagueId.current = "";
    routeHandoffApplied.current = true;
    setRequestedDraftId("");
    setActiveLeague(nextLeagueId || null);
    setDrafts([]);
    setDraftId("");
    setDraft(null);
    setPicks([]);
  ...
  useEffect(() => {
    let active = true;
    // A persisted league can hydrate before the URL handoff is committed.
    // Never fetch that stale league during the same effect cycle that applies
    // the Draft Monitor target.
    if (
      routeLeagueId.current &&
      String(activeLeague || "") !== String(routeLeagueId.current)
    ) return undefined;
  ...
  useEffect(() => {
    let active = true;
    const season = Number(draft?.season || league?.season || new Date().getFullYear());
    fetch(`/byes/${season}.json`).then((response) => response.ok ? response.json() : Promise.reject()).then((data) => {
      if (!active) return;
      const next = {};
      Object.entries(data?.by_week || {}).forEach(([week, teams]) => (teams || []).forEach((team) => {
        next[String(team).toUpperCase()] = Number(week);
      }));
      setByeWeeks(next);
    }).catch(() => active && setByeWeeks({}));
    return () => { active = false; };
  ...
  const focusCurrentRound = Math.ceil(focusCurrentPick / Math.max(1, teams));
  const finalRoundStashWindow = rounds > 0 && focusCurrentRound >= rounds;
  ...
  const qbRosterCount = needMap.get("QB")?.count || 0;
  const qbDepthCap = qbType === "sf" ? 3 : 2;
  const ranked = useMemo(() => eligible.slice(0, 160).map((item, index, list) => {
    ...
    const qb3Deferred = qbType === "sf" && item.pos === "QB" && need.count >= 3;
    const oneQbDepth = qbType === "1qb" && item.pos === "QB" && need.count >= 1;
    const oneQbQb3Deferred = oneQbDepth && need.count >= 2;
    const qbBeyondNormalDepth = item.pos === "QB" && need.count >= qbDepthCap;
    const exceptionalQbValue = qbBeyondNormalDepth && adpKnown && adpDelta <= -Math.max(20, teams * 2);
    const qbMultiplier = qb3Deferred
      ? exceptionalQbValue ? 0.62 : 0.3
      : oneQbQb3Deferred ? exceptionalQbValue ? 0.62 : 0.32
      : oneQbDepth ? fallingValue ? 0.82 : 0.68
      : 1;
    const availabilityMultiplier = item.injury.multiplier * byeMultiplier * qbMultiplier;
    const marketScore = item.value * (0.74 + valueRatio * 0.14 + timing * 0.12) * (likelyAvailableLater ? 0.78 : 1) * availabilityMultiplier;
    const unavailableStash = item.injury.severity === "unavailable";
    return { ...item, need, adpDelta, likelyAvailableLater, fallingValue, timing, byeWeek, sameBye, samePositionBye, byeMultiplier, qb3Deferred, oneQbDepth, oneQbQb3Deferred, qbBeyondNormalDepth, exceptionalQbValue, unavailableStash, availabilityMultiplier, marketScore, fitScore:marketScore * (0.82 + need.urgency * 0.18) * surplusPenalty, strategyScore:marketScore * (0.82 + need.urgency * 0.1 + scarcity * 0.08) * surplusPenalty * presetMultiplier };
  }), [byeWeeks, eligible, focusByeCounts, focusCurrentPick, followingMyPick, needMap, qbDepthCap, qbType, teams]);
  const takeDistinct = (rows, used, blockedPositions = new Set()) => rows.find((item) => !used.has(item.id) && !blockedPositions.has(item.pos));
  const recommendationPool = ranked.filter((item) => (!item.qbBeyondNormalDepth || item.exceptionalQbValue) && (!item.unavailableStash || finalRoundStashWindow));
  const valueRows = [...recommendationPool].sort((a, b) => b.marketScore - a.marketScore);
  const rosterFitPool = recommendationPool.filter((item) => item.pos !== "QB" || item.need.gap > 0 || (qbType === "sf" && qbRosterCount < 3));
  const fitRows = [...rosterFitPool].sort((a, b) => b.fitScore - a.fitScore);
  const strategyRows = [...rosterFitPool].sort((a, b) => b.strategyScore - a.strategyScore);
  ...
  const recommendedPositions = new Set(bestValue?.pos === "QB" ? ["QB"] : []);
  const bestFit = takeDistinct(fitRows, usedRecommendations, recommendedPositions); if (bestFit) { usedRecommendations.add(bestFit.id); if (bestFit.pos === "QB") recommendedPositions.add("QB"); }
  const bestStrategy = takeDistinct(strategyRows, usedRecommendations, recommendedPositions);
  ...
  const availabilityReason = (item) => {
    ...
    if (item?.unavailableStash) notes.push("NA players are treated as final-round stashes rather than normal draft recommendations");
    if (item?.byeWeek && item.sameBye >= 3) notes.push(`Week ${item.byeWeek} would become a ${item.sameBye + 1}-player bye cluster`);
    if (item?.qb3Deferred) notes.push("QB4 is normally deferred in Superflex unless the quarterback has fallen roughly two rounds beyond ADP");
    if (item?.oneQbDepth && !item.oneQbQb3Deferred) notes.push("a second quarterback receives reduced priority in this 1-QB format");
    if (item?.oneQbQb3Deferred) notes.push("QB3 is normally removed from recommendations in this 1-QB format; this player remains only because the ADP slide is exceptional");
  ...
  const byeSummary = useMemo(() => {
    ...
    ids.forEach((id) => {
      ...
      const week = byeWeeks[team];
      if (!week || !team) return;
      const row = weeks.get(week) || { week, count:0, teams:new Set() };
      row.count += 1;
      row.teams.add(team);
      weeks.set(week, row);
    });
    const top = [...weeks.values()].sort((a, b) => b.count - a.count)[0];
  ...
  return <main className="min-h-screen text-white"><BackgroundParticles /><Navbar pageTitle="Draft Command Center" /><div className="mx-auto max-w-[1500px] px-4 pb-20 pt-20">
    ...
    {draft ? <>
      ...
      {tab === "room" ? <DraftIntelligence nextMyPick={nextMyPick} picksAway={picksAway} recentRun={recentRun} tierSize={tierSize} valueCliff={valueCliff} draftedCounts={draftedCounts} byeSummary={byeSummary} topNeed={needs.find((need) => need.gap > 0)} /> : null}
