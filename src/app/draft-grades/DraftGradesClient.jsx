...
function buildGrades({ draft, picks, players, league, rosters, users, getMetric, getMarketRank, getMarketMeta, gradingKind="market" }) {
  ...
  const teams = [...teamMap.values()].map((team) => {
    ...
    const score = gradedPicks.length ? Math.round(clamp(avg*.78+construction*.2+balance*.02,35,100)) : null;
    ...
    return { ...team, score, grade:Number.isFinite(score)?grade(score):"—", pickAverage:avg, construction, lineupCompletion, depthScore, byeScore, balance, counts, totalValue:gradedPicks.reduce((sum,row)=>sum+row.value,0), best, reach, identity, gradedCount:gradedPicks.length, coverage:gradedPicks.length/Math.max(1,team.picks.length) };
  }).sort((a,b)=>n(b.score)-n(a.score) || b.totalValue-a.totalValue);
...
export default function DraftGradesClient() {
  ...
  return <main className="min-h-screen text-white"><BackgroundParticles/><Navbar pageTitle="Draft Grade Studio"/><div className="mx-auto max-w-7xl px-4 pb-20 pt-20">
    ...
    {analysis?<><section className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-6"><Metric label="Draft champion" value={analysis.teams[0]?.gradedCount?analysis.teams[0]?.name:"Insufficient data"} detail={analysis.teams[0]?.gradedCount?`${analysis.teams[0]?.grade} · ${analysis.teams[0]?.score}/100`:"No covered picks"} tone="green"/><Metric label="Best selection" value={analysis.steals[0]?playerName(analysis.steals[0]?.player):"—"} detail={analysis.steals[0]?`Pick #${analysis.steals[0].pick.pick_no}`:""} tone="cyan"/><Metric label="Largest reach" value={analysis.reaches[0]?playerName(analysis.reaches[0]?.player):"—"} detail={analysis.reaches[0]?`Pick #${analysis.reaches[0].pick.pick_no}`:""} tone="rose"/><Metric label="Draft pool" value={analysis.rookieOnly?"Rookies":"Full pool"} detail={`${analysis.eligibleCount} ranked candidates`}/><Metric label="Grade coverage" value={`${Math.round(analysis.coverage*100)}%`} detail={`${analysis.gradedCount} of ${analysis.pickRows.length} selections`}/><Metric label="Position runs" value={analysis.runs.length} detail="Four of six picks" tone="amber"/></section>
      ...
      {tab==="teams"&&selectedTeam?<div className="mt-4 grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]"><Panel className="p-4"><label><span className="mb-1 block text-xs text-white/38">Team report</span><select value={selectedTeam.rosterId} onChange={(event)=>setTeamId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-3">{analysis.teams.map((team)=><option key={team.rosterId} value={team.rosterId}>#{team.rank} · {team.name}</option>)}</select></label><div className="mt-5 text-center"><div className={`text-7xl font-black ${selectedTeam.gradedCount?gradeTone(selectedTeam.score):"text-white/30"}`}>{selectedTeam.grade}</div><div className="mt-1 text-sm text-white/35">{selectedTeam.gradedCount?`${selectedTeam.score}/100 overall`:"Insufficient source coverage"}</div></div><div className="mt-5 grid grid-cols-2 gap-2"><Metric label="Selection quality" value={selectedTeam.gradedCount?selectedTeam.pickAverage.toFixed(0):"—"} detail={`${selectedTeam.gradedCount}/${selectedTeam.picks.length} picks graded`}/><Metric label="Construction" value={selectedTeam.construction.toFixed(0)}/><Metric label="Coverage" value={`${Math.round(selectedTeam.coverage*100)}%`}/><Metric label="Identity" value={selectedTeam.identity?.[0]||"Balanced"} detail={selectedTeam.identity?`${selectedTeam.identity[1]} selected`:""}/></div><p className="mt-4 text-xs leading-5 text-white/42">{!selectedTeam.gradedCount?"This team is not ranked because the selected source does not cover its picks.":selectedTeam.score>=90?"An elite blend of selection value and roster construction.":selectedTeam.score>=80?"A strong draft with more wins than reaches and a credible starting structure.":selectedTeam.score>=70?"A mixed class with useful selections but identifiable opportunity cost.":"The selected lens exposes several reaches or construction gaps; review confidence and context before treating hindsight as process."}</p></Panel><Panel className="overflow-hidden"><div className="border-b border-white/10 p-4"><h3 className="text-xl font-black">Pick-by-pick report card</h3></div><div className="divide-y divide-white/[0.06]">{selectedTeam.picks.map((row)=><div key={row.pick.pick_no} className="grid grid-cols-[46px_minmax(0,1fr)_70px] items-center gap-3 p-3 sm:p-4"><b className="text-xs text-violet-100">#{row.pick.pick_no}</b><div className="min-w-0"><div className="truncate font-bold">{playerName(row.player,row.pick.player_id)}</div><div className="mt-1 text-[10px] text-white/32">{row.pos} · {row.verdict}{row.covered?` · ${row.gradingKind==="adp"?"ADP":"rank"} #${row.rank}`:""}</div></div><div className="text-right"><b className={`text-xl ${row.covered?gradeTone(row.score):"text-white/30"}`}>{row.grade}</b><small className="block text-[8px] text-white/25">{row.covered?`${row.score}/100`:"Excluded"}</small></div></div>)}</div></Panel></div>:null}
