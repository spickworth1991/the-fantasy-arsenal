...
function buildAudit({ league, matchups, transactions, tradedPicks, players, valueFor, pickValueFor, throughWeek }) {
  ...
  managerRows.forEach((manager) => {
    ...
    if (manager.emptyLineups >= 1) manager.reviewSignals.push({ type: "fact", label: "Empty starting position", detail: `${manager.emptyLineups} week${manager.emptyLineups === 1 ? "" : "s"} contained at least one empty starter position.` });
...
export default function CommissionerDashboardClient() {
  ...
  return <main className="min-h-screen text-white"><BackgroundParticles /><Navbar pageTitle="Commissioner Dashboard" /><div className="mx-auto max-w-7xl px-4 pb-16 pt-20">
    ...
    {username ? <Shell className="mt-4 p-4"><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(300px,440px)] lg:items-center"><div><div className="text-[11px] font-semibold uppercase tracking-[.2em] text-cyan-200/50">Roster strength model</div><div className="mt-1 text-lg font-black">Choose values or season projections</div><p className="mt-1 text-xs leading-5 text-white/42">Value sources measure market strength. Projection sources provide a current-season scoring lens. The selection controls roster ranks, positional profiles, trade estimates, orphan evaluations, and reports; activity metrics remain unchanged.</p><div className="mt-2 text-[11px] text-white/35">Currently using <span className="font-semibold text-white/65">{DEFAULT_SOURCES.find((source) => source.key === commissionerSourceKey)?.label || commissionerSourceKey}</span> · {commissionerMetricType === "projection" ? "Season projections" : `${format === "redraft" ? "Redraft" : "Dynasty"} · ${qbType === "1qb" ? "1QB" : "Superflex"}`}</div></div><SourceSelector value={commissionerSourceKey} onChange={setSourceKey} sources={DEFAULT_SOURCES} mode={format} qbType={qbType} onModeChange={setFormat} onQbTypeChange={setQbType} label="Commissioner roster source" layout="inline" /></div></Shell> : null}
    ...
    {data ? <><section className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-6"><Metric label="Health score" value={`${Math.round(data.healthScore)}/100`} detail="Composite participation and balance signal" tone={data.healthScore >= 75 ? "good" : data.healthScore >= 55 ? "warn" : "risk"} /><Metric label="Balance" value={`${Math.round(data.balanceScore)}/100`} detail={`${data.valueSpread.toFixed(1)}× top-to-bottom value`} /><Metric label="Balance trend" value={data.parityTrend} detail="Early weeks compared with recent weeks" /><Metric label="Participation" value={percent(data.participation * 100)} detail="Managers with recorded activity" /><Metric label="Needs review" value={data.attentionCount} detail="Manager and trade signals" tone={data.attentionCount ? "warn" : "good"} /><Metric label="Open rosters" value={data.managers.filter((manager) => manager.orphan).length} detail="No Sleeper owner assigned" tone={data.managers.some((manager) => manager.orphan) ? "risk" : "good"} /></section>
      ...
      {tab === "command" ? <CommissionerCommandCenter league={league} data={data} players={players} activityContent={<CommissionerActivityTracker league={league} data={data} players={players} />} reviewContent={<><CommissionerTradeRelationships data={data} /><CommissionerReviewCenter league={league} data={data} players={players} valueFor={valueFor} pickValueFor={pickValueFor} sourceLabel={selectedValueSource?.label || commissionerSourceKey} /></>} orphanContent={<OrphanEvaluator report={report} evaluator={evaluator} managers={data.managers} recruiting={recruiting} updateRecruiting={updateRecruiting} copied={copied} copyReport={copyReport} printReport={printReport} setReportRosterId={setReportRosterId} />} actionContent={<CommissionerActionQueue league={league} data={data} />} /> : null}
