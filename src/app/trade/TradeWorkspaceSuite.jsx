...
export default function TradeWorkspaceSuite({
  ...
  useEffect(() => {
    try {
      setSaved(JSON.parse(localStorage.getItem(`tfa:trade-workspaces:${leagueKey}`) || "[]"));
  ...
  useEffect(() => {
    ...
    if (!activeLeagueId || !["history", "market"].includes(tab) || historyLoadedLeague === activeLeagueId || historyLoading) return;
    let active = true;
    ...
    Promise.all(Array.from({ length:19 }, (_, index) => index).map((week) => fetch(`https://api.sleeper.app/v1/league/${activeLeagueId}/transactions/${week}`).then((response) => response.ok ? response.json() : []).catch(() => [])))
      .then((weeks) => {
        if (!active) return;
        setHistory(weeks.flat().filter((transaction) => transaction.type === "trade" && String(transaction.status || "").toLowerCase() === "complete").sort((a, b) => n(b.created) - n(a.created)));
        setHistoryLoadedLeague(activeLeagueId);
      })
      .finally(() => active && setHistoryLoading(false));
    return () => { active = false; };
  }, [league?.league_id, tab, historyLoadedLeague, historyLoading]);
  ...
  return <Panel className="mb-6 overflow-hidden border-violet-300/15">
    ...
    <div className="overflow-x-auto border-b border-white/10 p-2"><div className="flex w-max gap-1">{[["finder","Partner Finder"],["workspace","Workspace"],["block","Trade Block"],["history","History"],["market","League Market"],["swipe","Swipe Finder"]].map(([key, label]) => <button type="button" key={key} onClick={() => setTab(key)} className={`min-h-11 rounded-xl px-4 text-sm font-semibold ${tab === key ? "bg-violet-300/10 text-violet-100" : "text-white/40"}`}>{label}</button>)}</div></div>
    <div className="p-3 sm:p-4">
      {message ? <div className="mb-3 rounded-xl bg-cyan-300/[0.05] p-3 text-xs text-cyan-100">{message}</div> : null}
      {tab === "finder" ? <TradePartnerFinder league={league} players={players} getMetric={getMetric} metricMode={metricMode} username={username} onLoadPackage={onLoadPackage} /> : null}
      {tab === "workspace" ? <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="space-y-4"><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><Stat label="Side A" value={Math.round(sideA.reduce((sum, player) => sum + n(getMetric(player)), 0)).toLocaleString()} /><Stat label="Side B" value={Math.round(sideB.reduce((sum, player) => sum + n(getMetric(player)), 0)).toLocaleString()} tone="violet" /><Stat label="Roster delta" value={`${sideB.length - sideA.length >= 0 ? "+" : ""}${sideB.length - sideA.length}`} detail="Side A incoming slots" tone="amber" /><Stat label="Playoff handoff" value="Ready" detail="Open Scenario Explorer after package" tone="emerald" /></div><CardEditor sideA={sideA} sideB={sideB} getMetric={getMetric} inspect={inspect} /><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Negotiation notes, priorities, counters, and manager context…" className="min-h-28 w-full rounded-2xl border border-white/10 bg-slate-950 p-3 text-sm" /><div className="flex flex-wrap gap-2"><button type="button" onClick={saveWorkspace} className="rounded-xl bg-violet-300/10 px-4 py-3 text-xs font-black text-violet-100">Save package version</button><a href="/playoff-odds" className="rounded-xl bg-cyan-300/10 px-4 py-3 text-xs font-black text-cyan-100">Check playoff impact →</a></div></div><div><h3 className="font-black">Saved versions</h3><div className="mt-3 space-y-2">{saved.map((item) => <div key={item.id} className="rounded-2xl bg-black/15 p-3"><div className="flex items-center justify-between gap-2"><b className="text-xs">{new Date(item.createdAt).toLocaleString()}</b><select value={item.outcome} onChange={(event) => updateSaved(item.id, { outcome:event.target.value })} className="rounded-lg border border-white/10 bg-slate-950 px-2 py-1 text-[10px]"><option>Open</option><option>Sent</option><option>Countered</option><option>Accepted</option><option>Rejected</option></select></div><div className="mt-2 text-[10px] text-white/38">{item.sideA.map((id) => playerName(players?.[id], id)).join(" + ") || "Empty"} ↔ {item.sideB.map((id) => playerName(players?.[id], id)).join(" + ") || "Empty"}</div><div className="mt-2 flex justify-between"><span className="text-[9px] text-white/25">{item.notes || "No notes"}</span><button type="button" onClick={() => onLoadPackage?.(item.sideA.map((id) => players?.[id]).filter(Boolean), item.sideB.map((id) => players?.[id]).filter(Boolean), item.ownerA, item.ownerB)} className="text-[10px] text-cyan-100">Load →</button></div></div>)}{!saved.length ? <div className="text-xs text-white/35">Save multiple versions without replacing your current package.</div> : null}</div></div></div> : null}
      {tab === "block" ? <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><div><h3 className="font-black">Players available for discussion</h3><p className="mt-1 text-xs text-white/35">Stored locally. Nothing is written to Sleeper.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{myPlayers.map((player) => <button type="button" key={player.player_id} onClick={() => toggleBlock(String(player.player_id))} className={`flex items-center gap-2 rounded-xl border p-2 text-left ${block.includes(String(player.player_id)) ? "border-violet-300/25 bg-violet-300/[0.07]" : "border-white/[0.07] bg-black/15"}`}><AvatarImage name={playerName(player)} playerId={player.player_id} size={34} className="rounded-lg" alt="" /><div className="min-w-0 flex-1"><div className="truncate text-xs font-semibold">{playerName(player)}</div><div className="text-[9px] text-white/28">{position(player)} · {Math.round(n(getMetric(player))).toLocaleString()}</div></div><span>{block.includes(String(player.player_id)) ? "✓" : "+"}</span></button>)}</div></div><div className="space-y-4"><div><h3 className="font-black">Desired return</h3><div className="mt-3 flex flex-wrap gap-2">{["QB","RB","WR","TE","PICKS"].map((value) => <button type="button" key={value} onClick={() => setWanted((current) => current.includes(value) ? current.filter((item) => item !== value) : [...current, value])} className={`rounded-full px-3 py-1.5 text-xs ${wanted.includes(value) ? "bg-cyan-300/10 text-cyan-100" : "bg-white/[0.04] text-white/35"}`}>{value}</button>)}</div></div><button type="button" onClick={shareBlock} disabled={!block.length} className="w-full rounded-xl bg-violet-300/10 px-4 py-3 text-xs font-black text-violet-100 disabled:opacity-40">Copy private board link</button><div className="rounded-2xl bg-white/[0.025] p-4"><h3 className="font-black">Compatible managers</h3><div className="mt-3 space-y-2">{owners.map((roster) => { const needs=rosterNeeds(roster).filter((need) => need.gap > 0);const match=block.some((id) => needs.some((need) => need.pos === position(players?.[id])));return match ? <div key={roster.roster_id} className="rounded-xl bg-black/15 p-3 text-xs"><b>{ownerName(league, roster.owner_id)}</b><div className="mt-1 text-white/35">Needs {needs.slice(0, 2).map((need) => need.pos).join(" / ")}</div></div> : null; })}</div></div></div></div> : null}
