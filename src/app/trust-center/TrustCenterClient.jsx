...
function ModelTransparency({evidence,metrics}){
  ...
  return <div className="mt-4 space-y-4">
    <Panel className="overflow-hidden p-5 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div><div className="flex items-center gap-2"><h2 className="text-2xl font-black">Arsenal model evidence</h2><Type type="Fact"/></div><p className="mt-2 max-w-3xl text-xs leading-5 text-white/42">This is the proof behind the weekly stat model: exact training populations, pre-2025 tuning, untouched 2025 holdout performance, immutable inputs, probability calibration, and release warnings. A challenger ships only after beating the incumbent on both MAE and RMSE and passing the live adjustment-distribution gate.</p></div>
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{positions.map(([position,row])=><div key={position} className="rounded-2xl border border-white/[0.07] bg-black/15 p-4"><div className="flex items-center justify-between gap-2"><b>{position}</b><span className="text-[9px] font-black text-emerald-100">-{num(row.holdout_mae_improvement).toFixed(3)} MAE</span></div><div className="mt-2 text-xl font-black">{num(row.holdout_baseline?.mae).toFixed(2)} &rarr; {num(row.holdout_trained?.mae).toFixed(2)}</div><div className="mt-1 text-[9px] text-white/28">{num(row.validation_sample).toLocaleString()} untouched 2025 games</div><div className="mt-2 text-[8px] font-black uppercase tracking-wider text-cyan-100/45">{row.promotion?.promoted?"Challenger promoted":"Validated incumbent retained"}</div></div>)}</div>
    </Panel>
    <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
      <Panel className="p-5"><div className="flex items-center justify-between gap-3"><h3 className="text-xl font-black">What the model actually knows</h3><Type type="Estimate"/></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{[["Role and usage","Recent form, snap share, snap trend, opportunity share and trend."],["Scoring opportunity","Red-zone share and receiving air-yard share when captured."],["Game environment","Team play pace, home/away, opponent field-level tendencies and real weather inside its forecast window."],["Reliability controls","Position-specific regularization, missing-value neutrality, bounded application strength and empirical residual ranges."]].map(([title,detail])=><div key={title} className="rounded-2xl border border-white/[0.06] bg-black/15 p-4"><b className="text-sm">{title}</b><p className="mt-1 text-[10px] leading-4 text-white/38">{detail}</p></div>)}</div><div className="mt-4 rounded-2xl border border-amber-300/12 bg-amber-300/[0.04] p-4 text-[10px] leading-5 text-amber-100/60"><b>Not yet claimed:</b> archived historical betting lines, defensive injury units, route participation, pressure rate and receiver alignment are not silently estimated. Their adapters can join only after a lawful, timestamped provider is configured.</div></Panel>
      <Panel className="p-5"><div className="flex items-center justify-between gap-3"><h3 className="text-xl font-black">Probability scorecard</h3><Type type="Fact"/></div>{outcome?.sample?<div className="mt-4 grid grid-cols-2 gap-2"><Metric label="Boom Brier" value={num(outcome.boom_brier).toFixed(3)} detail="Lower is better"/><Metric label="Bust Brier" value={num(outcome.bust_brier).toFixed(3)} detail="Lower is better"/><Metric label="Range coverage" value={`${(num(outcome.floor_ceiling_coverage)*100).toFixed(0)}%`} detail={`${num(outcome.interval_sample)} final games`}/><Metric label="Calibration sample" value={num(outcome.sample).toLocaleString()} detail="Frozen forecasts"/></div>:<div className="mt-4 rounded-2xl border border-cyan-300/12 bg-cyan-300/[0.04] p-4 text-xs leading-5 text-cyan-100/55">The probability framework is calibrated on 2023–2025 history. Live Brier scores and interval coverage begin only after 2026 pre-kickoff snapshots have final results.</div>}</Panel>
    </div>
    <Panel className="p-5"><details><summary className="cursor-pointer text-lg font-black">Pipeline checks and current warnings</summary><div className="mt-4 grid gap-2 md:grid-cols-2">{(audit?.checks||[]).map((check)=><div key={check.key} className="rounded-2xl border border-white/[0.07] bg-black/15 p-4"><div className="flex items-center justify-between gap-2"><b className="text-xs">{String(check.key).replaceAll("_"," ")}</b><span className={`rounded-full border px-2 py-1 text-[8px] font-black ${check.status==="pass"?tone.fresh:check.status==="warn"?tone.watch:tone.stale}`}>{check.status}</span></div><p className="mt-2 text-[10px] leading-4 text-white/38">{check.message}</p></div>)}</div></details></Panel>
    <div className="grid gap-4 xl:grid-cols-2"><Panel className="p-5"><h2 className="text-xl font-black">Projection disagreement</h2><p className="mt-2 text-xs text-white/38">Largest ranges among players covered by at least three projection sources.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{metrics.disagreements.slice(0,10).map((row)=><div key={row.name} className="rounded-xl bg-violet-300/[0.04] p-3"><div className="truncate text-xs font-bold">{row.displayName||row.name||"Unknown player"}</div><div className="mt-1 text-lg font-black text-violet-100">{row.spread.toFixed(0)}%</div><div className="text-[9px] text-white/28">source range / mean</div></div>)}</div></Panel><Panel className="p-5"><h2 className="text-xl font-black">Value disagreement</h2><p className="mt-2 text-xs text-white/38">Largest normalized market ranges for the selected value format.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{metrics.valueDisagreements.slice(0,10).map((row)=><div key={row.name} className="rounded-xl bg-amber-300/[0.04] p-3"><div className="truncate text-xs font-bold">{row.displayName||row.name||"Unknown player"}</div><div className="mt-1 text-lg font-black text-amber-100">{row.spread.toFixed(0)}%</div><div className="text-[9px] text-white/28">percentile range / mean</div></div>)}</div></Panel></div>
  </div>;
}
...
export default function TrustCenterClient(){
  ...
  const [modelEvidence,setModelEvidence]=useState({calibration:null,audit:null,accuracy:null});
  ...
  useEffect(()=>{let live=true;(async()=>{
    const [sourceRows,index,freshnessLedger,calibration,projectionAudit,projectionAccuracy]=await Promise.all([
      ...
      fetch("/stats/projections/model-calibration.json",{cache:"no-store"}).then((r)=>r.ok?r.json():null).catch(()=>null),
      fetch(`/stats/projections/${season}/audit.json`,{cache:"no-store"}).then((r)=>r.ok?r.json():null).catch(()=>null),
      fetch(`/stats/projections/${season}/accuracy.json`,{cache:"no-store"}).then((r)=>r.ok?r.json():null).catch(()=>null),
    ]);
    if(live){setRecords(sourceRows.map((source)=>{const ledger=freshnessLedger?.sources?.[source.freshnessKey];return {...source,updated:source.updated||ledger?.last_success_at||index?.updated_at||null,updateStatus:ledger?.status||"unknown",lastAttempt:ledger?.last_attempt_at||null,lastError:ledger?.last_error||""};}));setArchive(index);setModelEvidence({calibration, audit:projectionAudit, accuracy:projectionAccuracy});setLoading(false);}
