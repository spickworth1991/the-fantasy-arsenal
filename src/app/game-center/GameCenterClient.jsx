...
const isFinal = (game) => game?.statusState === "post" || String(game?.status || "").toLowerCase().startsWith("final");
const isGameActive = (game) => {
  const status = String(game?.status || "").toLowerCase();
  return !isFinal(game) && (game?.statusState === "in" || ["live","progress","quarter","halftime","q1","q2","q3","q4","ot"].some((value) => status.includes(value)));
};
const normalizeTeam = (value) => ({ WSH:"WAS", JAX:"JAC", OAK:"LV", SD:"LAC", STL:"LAR" })[String(value || "").toUpperCase()] || String(value || "").toUpperCase();
...
function PreseasonCommand({ games, exposureRows, liveMode }) {
  ...
  const exposureLeaders = [...exposureRows].sort((a, b) => b.impact - a.impact || a.name.localeCompare(b.name)).slice(0, 12);
  return <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
    <div className="space-y-4">
      <Panel className="overflow-hidden">
        <div className="border-b border-white/10 p-4 sm:p-5"><div className="text-[10px] font-semibold uppercase tracking-[.2em] text-emerald-200/55">Preseason live board</div><h2 className="mt-1 text-2xl font-black">NFL games and scores</h2><p className="mt-1 text-xs leading-5 text-white/38">Use live refresh to test kickoff grouping, game status, weather, portfolio exposure, and mobile presentation before regular-season fantasy matchups begin.</p></div>
        <div className="grid gap-2 p-3 sm:grid-cols-2">
          {games.map((game) => <div key={game.id} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4">
            <div className="flex items-start justify-between gap-3"><div><div className="font-black">{(game.teams || []).join(" vs ")}</div><div className="mt-1 text-[10px] text-white/35">{game.date ? new Date(game.date).toLocaleString([], { weekday:"short", hour:"numeric", minute:"2-digit" }) : "Kickoff unavailable"}</div></div><span className={`rounded-full px-2 py-1 text-[9px] font-black ${isGameActive(game) ? "bg-emerald-300/12 text-emerald-100" : isFinal(game) ? "bg-white/[0.06] text-white/50" : "bg-cyan-300/10 text-cyan-100"}`}>{game.status}</span></div>
            <div className="mt-3 grid grid-cols-2 gap-2">{(game.competitors || []).map((team) => <div key={`${game.id}-${team.team}`} className="rounded-xl bg-black/20 px-3 py-2"><div className="text-[9px] uppercase tracking-wider text-white/30">{team.homeAway}</div><div className="mt-0.5 flex items-center justify-between gap-2"><b>{team.team}</b><b className="text-xl text-emerald-100">{team.score}</b></div></div>)}</div>
            <div className="mt-3 text-[9px] leading-4 text-cyan-100/50">{weatherLine(game)}</div>
          </div>)}
          {!games.length ? <div className="p-5 text-sm text-white/38">No preseason games were returned for this week.</div> : null}
        </div>
      </Panel>
    </div>
    <div className="space-y-4">
      <Panel className="p-4"><h2 className="font-black">Preseason test status</h2><div className="mt-3 grid grid-cols-3 gap-2"><Stat label="Games" value={games.length} /><Stat label="Live" value={active} tone={active ? "emerald" : "white"} /><Stat label="Final" value={final} /></div><p className="mt-3 text-[10px] leading-5 text-white/38">{liveMode ? "Automatic refresh is active using the same game-aware timing used on Sundays." : "Enter Live Mode to test automatic refresh and full-screen behavior."} Sleeper does not publish normal fantasy matchups for preseason, so no fake win probabilities, lineup grades, or 0–0 records are created.</p></Panel>
      <Panel className="overflow-hidden"><div className="border-b border-white/10 p-4"><h2 className="font-black">Most rostered preseason players</h2><p className="mt-1 text-[10px] text-white/32">Your portfolio exposure among players attached to this week&apos;s games.</p></div><div className="divide-y divide-white/[0.06]">{exposureLeaders.map((row) => <div key={row.id} className="flex items-center gap-3 p-3"><AvatarImage name={row.name} playerId={row.id} size={34} className="rounded-lg" alt="" /><div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold">{row.name}</div><div className="text-[9px] text-white/30">{row.player?.team} · {row.player?.position}</div></div><b className="text-cyan-100">{row.impact} <small className="text-[8px] font-normal text-white/30">league{row.impact === 1 ? "" : "s"}</small></b></div>)}{!exposureLeaders.length ? <div className="p-4 text-xs leading-5 text-white/35">Load a Sleeper portfolio to layer your roster exposure onto the preseason schedule.</div> : null}</div></Panel>
    </div>
  </div>;
}
...
export default function GameCenterClient() {
  ...
  const scan = useCallback(async (quiet = false) => {
    ...
    setError("");
    try {
      const schedulePromise = getJson(`/api/nfl-scoreboard?season=${year || new Date().getFullYear()}&week=${week}&seasonType=${seasonType}`).catch(() => ({ games:[] }));
      ...
      const scanned = root ? await concurrentMap(leagues, 12, async (league) => {
        try {
          const [rosters, users, matchups] = await Promise.all([
            ...
            seasonType === "preseason" ? Promise.resolve([]) : getJson(`https://api.sleeper.app/v1/league/${league.league_id}/matchups/${week}`),
          ]);
  ...
  useEffect(() => {
    if (nflStateReady && (seasonType === "preseason" || (username && leagues.length))) scan(false);
  ...
  const gameByTeam = useMemo(() => {
    const map = new Map();
    games.forEach((game) => (game.teams || []).forEach((team) => map.set(normalizeTeam(team), game)));
    return map;
  }, [games]);
  ...
  const preseasonPlayerRows = useMemo(() => {
    ...
    visibleRows.forEach((row) => {
      (row.mine?.players || []).map(String).forEach((id) => {
        ...
        const current = map.get(id) || { id, player, name:playerName(players, id), game, for:[], against:[], points:0, projection:0, conflict:false, preseasonExposure:true };
        current.for.push({ league:row.league });
        current.impact = current.for.length;
        map.set(id, current);
      });
    });
    return [...map.values()].sort((a, b) => String(a.game?.date || "9999").localeCompare(String(b.game?.date || "9999")) || b.impact - a.impact);
  }, [gameByTeam, players, preseasonMode, visibleRows]);
    ...
    <div className={`mx-auto max-w-[1600px] px-3 pb-20 sm:px-4 ${liveMode ? "pt-[max(1rem,env(safe-area-inset-top))]" : "pt-20"}`}>
      <header className="rounded-[30px] border border-emerald-300/15 bg-[radial-gradient(circle_at_88%_0%,rgba(16,185,129,.2),transparent_36%),radial-gradient(circle_at_8%_100%,rgba(34,211,238,.13),transparent_34%),linear-gradient(145deg,rgba(15,23,42,.98),rgba(2,6,23,.96))] p-4 sm:p-7">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-[.24em] text-emerald-200/60"><span>{preseasonMode ? "Preseason portfolio test center" : "Weekly portfolio command"}</span>{liveMode ? <span className="rounded-full bg-emerald-300/12 px-2 py-1 tracking-normal text-emerald-100">LIVE · {liveRefreshSeconds}s{activeGameCount ? ` · ${activeGameCount} game${activeGameCount === 1 ? "" : "s"} active` : " · waiting for kickoff"}</span> : null}</div>
            <h1 className="mt-2 text-3xl font-black sm:text-5xl">Fantasy Game Center</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/48">{preseasonMode ? "Test the live command experience with preseason schedules, team scores, game status, weather, and portfolio-wide roster exposure before fantasy matchups begin." : "One Sunday screen for scores, remaining players, lineup risk, late swaps, conflicts, weather, and the plays moving several leagues at once."}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => setLiveMode((value) => !value)} className={`min-h-11 rounded-2xl px-5 py-2 text-xs font-black ${liveMode ? "bg-rose-300/12 text-rose-100" : "bg-emerald-300/12 text-emerald-100"}`}>{liveMode ? "Exit Live Mode" : preseasonMode ? "Enter Preseason Live Mode" : "Enter Sunday Live Mode"}</button>
          </div>
        </div>
        <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-[150px_110px_170px_170px_1fr]">
          <select value={seasonType} onChange={(event) => { setSeasonType(event.target.value); setWeek(1); }} className="min-h-11 rounded-xl border border-white/10 bg-slate-950 px-3 text-sm"><option value="preseason">Preseason</option><option value="regular">Regular season</option></select>
          <select value={week} onChange={(event) => setWeek(n(event.target.value))} className="min-h-11 rounded-xl border border-white/10 bg-slate-950 px-3 text-sm">{Array.from({ length:preseasonMode ? 4 : 18 }, (_, index) => index + 1).map((value) => <option key={value} value={value}>Week {value}</option>)}</select>
      ...
      {!username ? <Panel className="mt-5 p-8 text-center text-white/50">{preseasonMode ? "Preseason games are available now. Load a Sleeper portfolio to add your player exposure across every league." : "Load a Sleeper portfolio to build its weekly command center."}</Panel> : null}

      {username || preseasonMode ? <>
        {preseasonMode ? <section className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6"><Stat label="Preseason games" value={games.length} detail={`Week ${week}`} tone="cyan" /><Stat label="Live now" value={activeGameCount} tone={activeGameCount ? "emerald" : "white"} /><Stat label="Final" value={games.filter(isFinal).length} /><Stat label="Portfolio players" value={preseasonPlayerRows.length} detail="Playing this week" tone="emerald" /><Stat label="League exposures" value={preseasonPlayerRows.reduce((sum, row) => sum + row.impact, 0)} detail="Rostered occurrences" tone="violet" /><Stat label="Refresh" value={liveMode ? `${liveRefreshSeconds}s` : "Manual"} detail={liveMode ? "Game-aware polling" : "Live Mode available"} tone="amber" /></section> : <section className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
        ...
        <Panel className="sticky top-0 z-30 mt-4 overflow-x-auto rounded-2xl bg-slate-950/95 p-2 backdrop-blur-xl">
          <div className="flex w-max gap-1">{(preseasonMode ? [["command", "Preseason Command"], ["timeline", "Player Exposure"]] : [["command", "Command"], ["timeline", "Player Timeline"], ["matchups", "Matchups"]]).map(([key, label]) => <button key={key} type="button" onClick={() => setTab(key)} className={`min-h-11 rounded-xl px-4 py-2 text-sm font-semibold ${tab === key ? "bg-white/10 text-white" : "text-white/42"}`}>{label}</button>)}</div>
        </Panel>

        {tab === "command" && preseasonMode ? <PreseasonCommand games={gameGroups} exposureRows={preseasonPlayerRows} liveMode={liveMode} /> : null}

        {tab === "command" && !preseasonMode ? <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
          <div className="space-y-4">
            <Panel className="overflow-hidden">
        ...
        {tab === "timeline" ? <div className="mt-4 space-y-4">
          <Panel className="p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-black">{preseasonMode ? "Preseason player exposure" : "Player kickoff timeline"}</h2><p className="mt-1 text-xs text-white/35">{preseasonMode ? "Every rostered portfolio player attached to this week&apos;s NFL games, grouped by kickoff window." : "Every involved player grouped by NFL window with live points and portfolio direction."}</p></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search player or team" className="min-h-11 rounded-xl border border-white/10 bg-slate-950 px-3 text-sm" /></div></Panel>
          {gameGroups.map((game) => {
