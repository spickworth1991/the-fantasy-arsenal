          ...
          <div className="space-y-4">
            <Panel className="overflow-hidden">
              ...
              <div className="divide-y divide-white/[0.06]">
                {riskyLineups.sort((a, b) => b.emptySlots - a.emptySlots || b.riskyStarters.length - a.riskyStarters.length).map((row) => <a key={row.league.league_id} href={matchupHref(row.league.league_id)} target="_blank" rel="noreferrer" className="flex min-h-16 items-center gap-3 p-3 transition hover:bg-white/[0.04] sm:p-4"><div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${row.emptySlots ? "bg-rose-300/10 text-rose-100" : "bg-amber-300/10 text-amber-100"}`}>{row.emptySlots ? "!" : "⚕"}</div><div className="min-w-0 flex-1"><div className="truncate font-bold">{row.league.name}</div><div className="mt-1 text-xs leading-5 text-white/38">{row.emptySlots ? `${row.emptySlots} empty starting slot${row.emptySlots === 1 ? "" : "s"}` : ""}{row.emptySlots && row.riskyStarters.length ? " · " : ""}{row.riskyStarters.map((id) => `${playerName(players, id)} (${injury(players?.[id]) || "inactive"})`).join(" · ")}</div></div><span className="text-[10px] font-semibold text-cyan-100">Open lineup ↗</span></a>)}
            ...
            <Panel className="overflow-hidden">
              ...
              <div className="grid gap-2 p-3 sm:grid-cols-2">
                {lateSwaps.slice(0, 12).map(({ starterId, replacement, row }) => <a key={`${row.league.league_id}-${starterId}`} href={matchupHref(row.league.league_id)} target="_blank" rel="noreferrer" className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-3 transition hover:bg-white/[0.055]"><div className="text-[9px] font-semibold uppercase tracking-wider text-white/30">{row.league.name}</div><div className="mt-2 text-sm"><b className="text-amber-100">{playerName(players, starterId)}</b><span className="mx-2 text-white/20">→</span><b className="text-emerald-100">{playerName(players, replacement)}</b></div><div className="mt-1 text-[10px] text-white/32">{injury(players?.[starterId])} contingency · {weeklyProjection(replacement).toFixed(1)} projected</div></a>)}
