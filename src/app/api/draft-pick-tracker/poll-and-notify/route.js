  ...
  try {
    ...
    for (const s of subs) {
      ...
      for (const draftId of s.draftIds) {
        ...
        // STACKING:
        // - Different leagues already separate by draftId
        // - Different thresholds should stack too → stage in tag
        const tag = `draft:${draftId}:pick:${nextPickNo}:stage:${stageToSend}`;
        ...
        const { endpoint, fetchInit } = await buildWebPushRequest({
          ...
          payload: {
            ...
            // premium options consumed by sw.js below
            tag,
            renotify: true,
