...
export default function WeeklyPortfolioDigest(){
  ...
  useEffect(()=>{if(!username||!leagues.length)return;let live=true;setLoading(true);setError("");(async()=>{
    ...
    const data=await Promise.all(leagues.map(async league=>{
      ...
      return {id:league.league_id,name:league.name,points,oppPoints,margin:points-oppPoints,result:points>oppPoints?"Winning":points<oppPoints?"Losing":"Tied",empty,opponent:oppUser?.display_name||oppUser?.username||"Opponent"};
    }));
  ...
  const summary=useMemo(()=>{const wins=rows.filter(r=>r.margin>0).length,losses=rows.filter(r=>r.margin<0).length,points=rows.reduce((s,r)=>s+r.points,0),close=rows.filter(r=>Math.abs(r.margin)<=10).length,empty=rows.reduce((s,r)=>s+r.empty,0);const score=Math.max(0,Math.min(100,70+(wins-losses)*4-empty*12));return{wins,losses,points,close,empty,score,grade:score>=93?"A":score>=90?"A−":score>=87?"B+":score>=83?"B":score>=80?"B−":score>=73?"C":"D",best:[...rows].sort((a,b)=>b.margin-a.margin)[0],worst:[...rows].sort((a,b)=>a.margin-b.margin)[0]};},[rows]);
  ...
  return <Panel className="overflow-hidden"><div className="border-b border-white/10 bg-[radial-gradient(circle_at_90%_0%,rgba(34,211,238,.14),transparent_42%)] p-5"><div className="text-[10px] font-bold uppercase tracking-[.22em] text-cyan-200/55">Week {week} · {season}</div><div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-black">Weekly Portfolio Digest</h2><p className="mt-1 text-xs leading-5 text-white/38">One account-level recap across every loaded league, with urgent problems and direct next steps.</p></div><div className="text-5xl font-black text-cyan-100">{summary.grade}</div></div></div>
    <div className="p-4 sm:p-5">{loading?<div className="text-sm text-white/40">Building this week’s portfolio story…</div>:error?<div className="text-sm text-rose-100">{error}</div>:<><div className="grid grid-cols-2 gap-2 lg:grid-cols-6"><Metric label="Record" value={`${summary.wins}-${summary.losses}`}/><Metric label="Points" value={summary.points.toFixed(1)}/><Metric label="Close games" value={summary.close}/><Metric label="Lineup zeros" value={summary.empty}/><Metric label="Best result" value={summary.best?`${summary.best.margin>=0?"+":""}${summary.best.margin.toFixed(1)}`:"—"} detail={summary.best?.name}/><Metric label="Biggest concern" value={summary.worst?`${summary.worst.margin.toFixed(1)}`:"—"} detail={summary.worst?.name}/></div><div className="mt-4 grid gap-3 md:grid-cols-2">{rows.sort((a,b)=>Math.abs(a.margin)-Math.abs(b.margin)).slice(0,8).map(r=><Link key={r.id} href={`/league-hub?league=${r.id}`} className="rounded-2xl border border-white/[0.07] bg-black/15 p-3 transition hover:bg-white/[0.04]"><div className="flex justify-between gap-3"><b className="truncate">{r.name}</b><span className={r.margin>=0?"text-emerald-100":"text-rose-100"}>{r.margin>=0?"+":""}{r.margin.toFixed(1)}</span></div><div className="mt-1 text-[10px] text-white/32">{r.points.toFixed(1)}–{r.oppPoints.toFixed(1)} vs {r.opponent}{r.empty?` · ${r.empty} empty slot`:""}</div></Link>)}</div><div className="mt-4 flex flex-wrap gap-2"><Link href="/game-center" className="rounded-xl bg-cyan-300/10 px-4 py-2 text-xs font-black text-cyan-100">Open live command center</Link><Link href="/lineup" className="rounded-xl bg-white/[0.05] px-4 py-2 text-xs font-black">Review lineups</Link><Link href="/player-availability" className="rounded-xl bg-white/[0.05] px-4 py-2 text-xs font-black">Find acquisitions</Link></div></>}</div>
