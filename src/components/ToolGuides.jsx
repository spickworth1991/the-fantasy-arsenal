...
const GUIDES = {
  ...
  "/trust-center": [
    ...
    s("main .sticky button:nth-of-type(2)", "Value Intelligence", "Compare market sources only where player identities overlap. Agreement increases confidence in consensus; disagreement is a research flag, not proof that one source is wrong.", tab("Value intelligence")),
    s("main .sticky button:nth-of-type(3)", "Trends", "Movement requires comparable stored snapshots. A missing prior snapshot stays unavailable rather than turning current rank into invented movement.", tab("Trends")),
    ...
    s("main .sticky button:nth-of-type(5)", "Source Ledger", "Inspect each source's fetch state, timestamp, mode, metric type, player count, and normalization coverage. Files using players or data arrays and points_ppr/projected_points fields are now recognized instead of being reported empty.", tab("Source ledger")),
    s("main .sticky button:nth-of-type(6)", "Model Transparency", "Review inputs, calibration, promotion gates, limitations, and stored artifacts behind Arsenal estimates. Validation evidence is versioned separately from current projections so a routine refresh does not rewrite the historical result. Closing returns to Trust Overview.", () => { tab("Model transparency")(); return () => tab("Trust overview")(); }),
  ],
  "/ballsville-stats": [
