...
function Metric({ label, value, detail, tone = "cyan" }) {
  ...
  return <div className="min-w-0 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-3"><div className="break-words text-[9px] font-semibold uppercase tracking-[.13em] text-white/30 sm:tracking-[.16em]">{label}</div><div className={`mt-1 break-words text-base font-black leading-tight min-[390px]:text-lg sm:text-xl ${colors}`}>{value}</div>{detail ? <div className="mt-1 break-words text-[10px] leading-4 text-white/35">{detail}</div> : null}</div>;
}
...
function SourceTable({ title, rows, currentSource, suffix = "" }) {
  ...
  return <section><div className="mb-2 flex flex-wrap items-end justify-between gap-2"><div><h3 className="text-sm font-black">{title}</h3><p className="text-[10px] text-white/30">{usable.length} reporting sources · {spread > .25 ? "high disagreement" : spread > .12 ? "moderate disagreement" : "tight consensus"}</p></div><div className="text-left text-[10px] text-white/35 min-[390px]:text-right">Consensus <b className="text-white/70">{average ? suffix ? average.toFixed(1) : Math.round(average).toLocaleString() : "—"}{average ? suffix : ""}</b></div></div><div className="mb-2 grid grid-cols-1 gap-2 min-[350px]:grid-cols-3"><Metric label="Floor" value={floor ? `${suffix ? floor.toFixed(1) : Math.round(floor).toLocaleString()}${suffix}` : "—"} /><Metric label="Consensus" value={average ? `${suffix ? average.toFixed(1) : Math.round(average).toLocaleString()}${suffix}` : "—"} tone="emerald" /><Metric label="Ceiling" value={ceiling ? `${suffix ? ceiling.toFixed(1) : Math.round(ceiling).toLocaleString()}${suffix}` : "—"} tone="violet" /></div><div className="overflow-hidden rounded-2xl border border-white/10"><table className="w-full table-fixed text-left text-[11px] sm:text-xs"><thead className="bg-white/[0.04] text-[9px] uppercase tracking-wider text-white/30"><tr><th className="w-[56%] px-2.5 py-2.5 sm:px-3">Source</th><th className="px-2.5 py-2.5 text-right sm:px-3">Result</th><th className="hidden px-3 py-2.5 text-right sm:table-cell">vs consensus</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{rows.map((row) => { const delta = average && row.amount ? (row.amount - average) / average : 0; const outlier = Math.abs(delta) >= .2; return <tr key={row.key} className={row.key === currentSource ? "bg-cyan-300/[0.055]" : ""}><td className="px-2.5 py-3 sm:px-3"><div className="break-words font-semibold leading-4 text-white/75">{row.label}</div><div className="text-[8px] font-semibold uppercase tracking-wider text-white/25">{row.key === currentSource ? "Current source" : outlier ? "Source outlier" : ""}</div></td><td className="break-words px-2.5 py-3 text-right font-black sm:px-3">{!row.supported ? <span className="font-normal text-white/25">Not offered</span> : row.amount > 0 ? `${suffix ? row.amount.toFixed(1) : Math.round(row.amount).toLocaleString()}${suffix}` : <span className="font-normal text-white/25">Unavailable</span>}</td><td className={`hidden px-3 py-3 text-right text-[10px] sm:table-cell ${outlier ? "text-amber-200" : "text-white/30"}`}>{row.amount && average ? `${delta >= 0 ? "+" : ""}${Math.round(delta * 100)}%` : "—"}</td></tr>; })}</tbody></table></div></section>;
}
...
export default function GlobalPlayerSourceDrawer() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      Promise.all([startWeek, Math.min(18, startWeek + 1)].map((week) => fetch(`/api/nfl-scoreboard?season=${scheduleSeason}&week=${week}`).then((response) => response.ok ? response.json() : { games:[] }).then((payload) => (payload.games || []).map((game) => {
        const indoor = Boolean(game.venue?.indoor);
        const roofType = game.venue?.roofType;
        const temperature = game.weather?.temperature ?? game.weather?.highTemperature;
        ...
        const conditions = indoor
          ...
          : game.weather
            ? [
                ...
                wind ? `Wind ${Math.round(wind)} mph${gusts > wind + 3 ? `, gusts ${Math.round(gusts)}` : ""}` : "",
                rainChance ? `${Math.round(rainChance)}% precipitation` : "",
              ].filter(Boolean).join(" · ")
            : "Forecast pending";
        const roofLabel = roofType === "retractable"
          ? "Retractable roof decision pending"
          : roofType === "canopy"
            ? "Covered field with open sides"
            : "";
        return { ...game, week, status:[game.status, roofLabel, conditions].filter(Boolean).join(" · ") };
      })))).then((games) => {
  ...
  return <div className="fixed inset-0 z-[110] flex h-[100dvh] max-h-[100dvh] items-end justify-center overflow-hidden overscroll-none bg-slate-950/85 backdrop-blur-md sm:items-center sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setPlayerId(""); }}><div role="dialog" aria-modal="true" aria-label={`Player intelligence for ${name}`} className="flex h-[100dvh] max-h-[100dvh] w-full flex-col overflow-hidden rounded-none border border-white/12 bg-[radial-gradient(circle_at_90%_0%,rgba(34,211,238,.1),transparent_26%),linear-gradient(160deg,#07101f,#020617_58%)] shadow-2xl sm:h-auto sm:max-h-[calc(100dvh-2rem)] sm:max-w-4xl sm:rounded-[30px]"><div className="z-20 shrink-0 border-b border-white/10 bg-slate-950/95 pt-[env(safe-area-inset-top)] backdrop-blur-xl sm:pt-0"><div className="flex items-center gap-3 px-3 pb-2 pt-3 sm:p-4"><AvatarImage inspectable={false} name={name} playerId={playerId} size={48} className="shrink-0 rounded-2xl ring-1 ring-white/10" alt="" /><div className="min-w-0 flex-1"><div className="truncate text-lg font-black sm:text-2xl">{name}</div><div className="line-clamp-2 text-[11px] leading-4 text-white/38 sm:text-xs">{player.position || "—"} · {player.team || "FA"} · {age ? `Age ${age}` : "Age unavailable"} · {format} · {qbType === "sf" ? "Superflex" : "1QB"}</div></div><button type="button" onClick={() => setPlayerId("")} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-white/[0.06] text-xl text-white/60 transition hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/60" aria-label="Close player intelligence">×</button></div><div className="flex snap-x snap-mandatory gap-1 overflow-x-auto overscroll-x-contain px-3 pb-3 [-webkit-overflow-scrolling:touch]">{TABS.map(([key, label]) => <button key={key} type="button" onClick={() => setTab(key)} className={`min-h-11 shrink-0 snap-start rounded-xl px-3 py-2 text-[11px] font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/60 ${tab === key ? "bg-cyan-300/12 text-cyan-100 ring-1 ring-cyan-300/20" : "text-white/38 hover:bg-white/[0.05] hover:text-white/70"}`}>{label}</button>)}</div></div><div className="min-h-0 flex-1 touch-pan-y space-y-4 overflow-y-auto overscroll-contain px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 [-webkit-overflow-scrolling:touch] sm:space-y-5 sm:p-5">
