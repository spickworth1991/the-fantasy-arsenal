...
// ../../src/durable/DraftRegistryDO.js
var TICK_MS = 15e3;
var ACTIVE_REFRESH_MS = 2e4;
var PRE_DRAFT_REFRESH_MS = 2 * 60 * 1e3;
var INACTIVE_REFRESH_MS = 6 * 60 * 60 * 1e3;
async function sleeperJson(url) {
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) throw new Error(`Sleeper fetch failed ${res.status} for ${url}`);
  return res.json();
}
__name(sleeperJson, "sleeperJson");
async function ensureDraftRegistryTable(db) {
  ...
  try {
    ...
    const add = /* @__PURE__ */ __name(async (name, type) => {
...
__name(ensureDraftRegistryTable, "ensureDraftRegistryTable");
...
__name(ensureDraftCacheTable, "ensureDraftCacheTable");
...
__name(loadDraftCache, "loadDraftCache");
...
__name(saveDraftCache, "saveDraftCache");
...
__name(getPickCount, "getPickCount");
...
__name(coerceJsonStr, "coerceJsonStr");
async function listUniqueDraftIdsFromSubs(db) {
  const rows = await db.prepare(`SELECT draft_ids_json FROM push_subscriptions WHERE draft_ids_json IS NOT NULL`).all();
...
__name(listUniqueDraftIdsFromSubs, "listUniqueDraftIdsFromSubs");
...
__name(listUniqueDraftIdsFromRegistry, "listUniqueDraftIdsFromRegistry");
async function listUniqueDraftIds(db) {
  ...
  return Array.from(/* @__PURE__ */ new Set([...subs || [], ...reg || []]));
}
__name(listUniqueDraftIds, "listUniqueDraftIds");
async function getRegistryRow(db, draftId) {
  return await db.prepare(
    `SELECT draft_id, active, status, last_checked_at, last_active_at, last_inactive_at,
         ...
         WHERE draft_id=?`
  ).bind(String(draftId)).first() || null;
}
__name(getRegistryRow, "getRegistryRow");
...
__name(upsertRegistry, "upsertRegistry");
function safeNum(v) {
  const x = typeof v === "number" ? v : Number(v);
  return Number.isFinite(x) ? x : 0;
}
__name(safeNum, "safeNum");
...
__name(getSnakeSlotForPick, "getSnakeSlotForPick");
...
__name(resolveRosterForPick, "resolveRosterForPick");
async function tickOnce(env) {
  ...
  for (const draftId of uniqueDraftIds) {
    ...
    const staleMs = wasActive ? ACTIVE_REFRESH_MS : statusLower === "pre_draft" ? PRE_DRAFT_REFRESH_MS : INACTIVE_REFRESH_MS;
      ...
      batch.map(async ({ draftId, wasActive, reg }) => {
        ...
        if (canHydrateRosterContext) {
          try {
            ...
            const ownerToName = /* @__PURE__ */ new Map();
            ...
            if (s2r && typeof s2r === "object") {
              ...
              const ownerToRoster = /* @__PURE__ */ new Map();
              for (const r of Array.isArray(rosters) ? rosters : []) {
            ...
            if (Object.keys(rosterNames || {}).length > 0 && Object.keys(rosterByUsername || {}).length > 0 && Object.keys(slotToRoster || {}).length > 0) {
              didHydrateRosterContext = true;
            }
          } catch {
          }
        }
        const shouldRefreshTraded = Number(bestBall || 0) !== 1 && Boolean(leagueId) && (isActive || contextStale || !tradedPickOwnerJson);
        ...
        const clockEndsAt = lastPicked && timerSec ? Number(lastPicked) + Number(timerSec) * 1e3 : null;
...
__name(tickOnce, "tickOnce");
var DraftRegistry = class {
  static {
    __name(this, "DraftRegistry");
  }
